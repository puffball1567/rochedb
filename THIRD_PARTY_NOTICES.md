...
This file tracks known third-party dependencies and tooling used by RocheDB.
...
- `drivers/csharp/RocheDB/RocheDB.csproj`
...
|---|---|---|---|---|
| Nim standard library | RocheDB core | MIT | Nim toolchain | Used by the Nim implementation. Not vendored. |
| nimsodium | RocheDB core auth/encryption path | MIT | `rochedb.nimble`: `nimsodium >= 0.2.0`; package metadata URL: `https://github.com/puffball1567/nimsodium` | Local Nimble package metadata reports MIT. Not vendored. |
...
| .NET runtime | C# driver | MIT | `drivers/csharp/RocheDB/RocheDB.csproj` | The C# wrapper has no NuGet package dependencies. |
| JVM / Kotlin standard library | Kotlin driver | Apache-2.0 for Kotlin components; JVM distribution varies | `drivers/kotlin/Dockerfile` downloads the Kotlin compiler for smoke tests | The Kotlin smoke path uses JNI and the RocheDB C ABI. |

## Build And Test Tooling

These tools are used to build or test RocheDB and its drivers. They are not
vendored or redistributed by the RocheDB source tree.

| Component | Scope | License | Source / declaration | Notes |
|---|---|---|---|---|
| Nim compiler / Nimble | Core build and tests | MIT | `rochedb.nimble`, local Nim toolchain | Required to build RocheDB from source. |
...
intended production vector backend path, but RocheDB loads it through a dynamic
...
|---|---|---|---|---|
| FAISS | Production vector backend bridge | MIT | Source checkout target: `third_party/faiss` via `scripts/fetch_faiss.sh`; bridge source: `src/roche/faiss_bridge.cpp`; RocheDB core loads `libroche_faiss.so` dynamically when `vbFaiss` is selected | Default tag: `v1.14.3`; last verified commit recorded in `third_party/faiss.version`. Users can pin an exact commit with `ROCHE_FAISS_COMMIT`. License text is available in the fetched checkout at `third_party/faiss/LICENSE`. FAISS is not vendored, not statically linked by the default core build, and not committed to this repository. See `docs/faiss-versioning.md`. |
...
- Docker images used for smoke tests are development artifacts; they should not
  be treated as RocheDB runtime redistribution packages.
