...
Minimal C++17 wrapper for OrbeliasDB through the stable C ABI.
...
The script builds `lib/liborbeliasdb.so` with TLS support enabled for the C ABI.

Build and run the smoke test:

```sh
g++ -std=c++17 -Iinclude -Idrivers/cpp/include drivers/cpp/examples/contract_smoke.cpp -Llib -lorbeliasdb -Wl,-rpath,'$ORIGIN/../../../lib' -o /tmp/orbelias_cpp_smoke
