...
const peers = process.env.ROCHE_TEST_PEERS || "127.0.0.1:17941,127.0.0.1:17942";
...
async function waitCluster(client: InstanceType<typeof RocheClient>): Promise<void> {
...
test("Bun driver roundtrips through the Node-compatible wire driver", async () => {
  ...
  try {
    ...
    expect(id).toBeInstanceOf(RocheId);
    ...
    expect(RocheId.parse(String(textId))).toEqual(textId);
