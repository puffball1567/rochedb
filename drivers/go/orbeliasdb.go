...
#cgo LDFLAGS: -L${SRCDIR}/../../lib -Wl,-rpath,${SRCDIR}/../../lib -lorbeliasdb
#include <stdlib.h>
...
func initRuntime() error {
	...
	if int(C.orbelias_abi_version()) != ABIVersion {
		return fmt.Errorf("orbeliasdb ABI version mismatch: expected %d, got %d",
			ABIVersion, int(C.orbelias_abi_version()))
	}
	return nil
}

func lastError() error {
	msg := C.orbelias_last_error()
	if msg == nil {
		return errors.New("orbeliasdb C ABI error")
	}
	text := C.GoString(msg)
	if text == "" {
		return errors.New("orbeliasdb C ABI error")
...
func Open(nodes int) (*DB, error) {
	...
	raw := C.orbelias_open(C.int(nodes))
...
func OpenDir(nodes int, dir string) (*DB, error) {
	...
	raw := C.orbelias_open_dir(C.int(nodes), cdir)
	return dbFromRaw(raw)
}
...
func ConnectAuth(peers, username, password, authToken, secretKey, galaxy string) (*DB, error) {
	...
	raw := C.orbelias_connect_auth(cpeers, cusername, cpassword, cauthToken, csecretKey, cgalaxy)
	return dbFromRaw(raw)
}
...
func (db *DB) Close() {
	...
	C.orbelias_close(db.raw)
	db.raw = nil
	runtime.SetFinalizer(db, nil)
}

func (db *DB) Now() float64 {
	return float64(C.orbelias_now(db.raw))
}

func (db *DB) Advance(dt float64) {
	C.orbelias_advance(db.raw, C.double(dt))
}

func (db *DB) ConfigureRing(ring string, period float64) error {
	cring := C.CString(ring)
	defer C.free(unsafe.Pointer(cring))
	if C.orbelias_ring_configure(db.raw, cring, C.double(period)) != C.ORBELIAS_OK {
		return lastError()
	}
	return nil
}
...
func (db *DB) SetGalaxyDescription(description string) error {
	...
	if C.orbelias_set_galaxy_description(db.raw, cdesc) != C.ORBELIAS_OK {
		return lastError()
	}
	return nil
}
...
func (db *DB) SetRingDescription(ring, description string) error {
	...
	defer C.free(unsafe.Pointer(cring))
	defer C.free(unsafe.Pointer(cdesc))
	if C.orbelias_set_ring_description(db.raw, cring, cdesc) != C.ORBELIAS_OK {
		return lastError()
	}
	return nil
}
...
func (db *DB) Put(ring string, payload []byte) (ID, error) {
	cring := C.CString(ring)
	defer C.free(unsafe.Pointer(cring))
	var out C.orbelias_id
	if C.orbelias_put(db.raw, cring, bytesPtr(payload), C.size_t(len(payload)), &out) != C.ORBELIAS_OK {
		return ID{}, lastError()
	}
...
func (db *DB) PutVec(ring string, payload []byte, vec []float32) (ID, error) {
	...
	defer C.free(unsafe.Pointer(cring))
	var out C.orbelias_id
	if C.orbelias_put_vec(db.raw, cring, bytesPtr(payload), C.size_t(len(payload)),
		floatPtr(vec), C.size_t(len(vec)), &out) != C.ORBELIAS_OK {
		return ID{}, lastError()
	}
	return idFromC(out), nil
}
...
func (db *DB) Get(id ID) ([]byte, bool, error) {
	var n C.size_t
	p := C.orbelias_get(db.raw, idToC(id), &n)
	if p == nil {
		err := lastError()
	...
	defer C.orbelias_free(p)
	return C.GoBytes(p, C.int(n)), true, nil
}

func (db *DB) BatchGet(ids []ID) ([][]byte, error) {
	cids := make([]C.orbelias_id, len(ids))
	for i, id := range ids {
		cids[i] = idToC(id)
	}
	var ptr *C.orbelias_id
	if len(cids) > 0 {
		ptr = &cids[0]
	}
	r := C.orbelias_batch_get(db.raw, ptr, C.size_t(len(cids)))
	if r == nil {
		return nil, lastError()
	}
	defer C.orbelias_batch_get_free(r)
...
func (db *DB) Query(id ID, selection string) ([]byte, error) {
	...
	var n C.size_t
	p := C.orbelias_query(db.raw, idToC(id), csel, &n)
	if p == nil {
		return nil, lastError()
	}
	defer C.orbelias_free(p)
...
func (db *DB) Retrieve(vec []float32, ring string, budget, topRings, focus int) (RetrieveResult, error) {
	cring := C.CString(ring)
	defer C.free(unsafe.Pointer(cring))
	r := C.orbelias_retrieve(db.raw, floatPtr(vec), C.size_t(len(vec)), cring,
		C.int(budget), C.int(topRings), C.int(focus))
	if r == nil {
		return RetrieveResult{}, lastError()
	}
	defer C.orbelias_retrieve_free(r)
...
func (db *DB) Atlas(queryVec []float32, maxCentroidDims int) (string, error) {
	var n C.size_t
	p := C.orbelias_atlas(db.raw, floatPtr(queryVec), C.size_t(len(queryVec)),
...
func (db *DB) Locate(id ID, at float64) (int, error) {
	node := C.orbelias_locate(db.raw, idToC(id), C.double(at))
	if node < 0 {
		return -1, lastError()
	}
	return int(node), nil
...
func (db *DB) NextVisit(id ID, node int) (float64, error) {
	t := C.orbelias_next_visit(db.raw, idToC(id), C.int(node))
	if t < 0 {
		return -1, lastError()
	}
...
func (db *DB) NextJoin(a, b ID) (float64, bool, error) {
	t := C.orbelias_next_join(db.raw, idToC(a), idToC(b))
	if t < 0 {
		return -1, false, nil
	}
...
func idToC(id ID) C.orbelias_id {
	return C.orbelias_id{
...
func idFromC(id C.orbelias_id) ID {
	return ID{
