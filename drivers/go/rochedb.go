...
#cgo LDFLAGS: -L${SRCDIR}/../../lib -Wl,-rpath,${SRCDIR}/../../lib -lrochedb
#include <stdlib.h>
#include "rochedb.h"
*/
import "C"
...
func initRuntime() error {
	...
	if int(C.roche_abi_version()) != ABIVersion {
		return fmt.Errorf("rochedb ABI version mismatch: expected %d, got %d",
			ABIVersion, int(C.roche_abi_version()))
	}
	return nil
}

func lastError() error {
	msg := C.roche_last_error()
	if msg == nil {
		return errors.New("rochedb C ABI error")
	}
	text := C.GoString(msg)
	if text == "" {
		return errors.New("rochedb C ABI error")
...
func OpenDir(nodes int, dir string) (*DB, error) {
	...
	raw := C.roche_open_dir(C.int(nodes), cdir)
	return dbFromRaw(raw)
}
...
func ConnectAuth(peers, username, password, authToken, secretKey, galaxy string) (*DB, error) {
	...
	raw := C.roche_connect_auth(cpeers, cusername, cpassword, cauthToken, csecretKey, cgalaxy)
	return dbFromRaw(raw)
}
...
func (db *DB) SetGalaxyDescription(description string) error {
	...
	if C.roche_set_galaxy_description(db.raw, cdesc) != C.ROCHE_OK {
		return lastError()
	}
	return nil
}
...
func (db *DB) SetRingDescription(ring, description string) error {
	...
	defer C.free(unsafe.Pointer(cring))
	defer C.free(unsafe.Pointer(cdesc))
	if C.roche_set_ring_description(db.raw, cring, cdesc) != C.ROCHE_OK {
		return lastError()
	}
	return nil
}
...
func (db *DB) Put(ring string, payload []byte) (ID, error) {
	...
	defer C.free(unsafe.Pointer(cring))
	var out C.roche_id
	if C.roche_put(db.raw, cring, bytesPtr(payload), C.size_t(len(payload)), &out) != C.ROCHE_OK {
		return ID{}, lastError()
	}
...
func (db *DB) PutVec(ring string, payload []byte, vec []float32) (ID, error) {
	...
	defer C.free(unsafe.Pointer(cring))
	var out C.roche_id
	if C.roche_put_vec(db.raw, cring, bytesPtr(payload), C.size_t(len(payload)),
		floatPtr(vec), C.size_t(len(vec)), &out) != C.ROCHE_OK {
		return ID{}, lastError()
	}
	return idFromC(out), nil
}
...
func (db *DB) Get(id ID) ([]byte, bool, error) {
	var n C.size_t
	p := C.roche_get(db.raw, idToC(id), &n)
	if p == nil {
		err := lastError()
...
func (db *DB) BatchGet(ids []ID) ([][]byte, error) {
	...
	r := C.roche_batch_get(db.raw, ptr, C.size_t(len(cids)))
	if r == nil {
		return nil, lastError()
	}
	defer C.roche_batch_get_free(r)
...
func (db *DB) Query(id ID, selection string) ([]byte, error) {
	...
	var n C.size_t
	p := C.roche_query(db.raw, idToC(id), csel, &n)
	if p == nil {
		return nil, lastError()
	}
	defer C.roche_free(p)
...
func (db *DB) Retrieve(vec []float32, ring string, budget, topRings, focus int) (RetrieveResult, error) {
	cring := C.CString(ring)
	defer C.free(unsafe.Pointer(cring))
	r := C.roche_retrieve(db.raw, floatPtr(vec), C.size_t(len(vec)), cring,
		C.int(budget), C.int(topRings), C.int(focus))
	if r == nil {
		return RetrieveResult{}, lastError()
	}
	defer C.roche_retrieve_free(r)
...
func (db *DB) Atlas(queryVec []float32, maxCentroidDims int) (string, error) {
	var n C.size_t
	p := C.roche_atlas(db.raw, floatPtr(queryVec), C.size_t(len(queryVec)),
...
func (db *DB) Locate(id ID, at float64) (int, error) {
	node := C.roche_locate(db.raw, idToC(id), C.double(at))
	if node < 0 {
		return -1, lastError()
	}
	return int(node), nil
...
func (db *DB) NextVisit(id ID, node int) (float64, error) {
	t := C.roche_next_visit(db.raw, idToC(id), C.int(node))
	if t < 0 {
		return -1, lastError()
	}
...
func (db *DB) NextJoin(a, b ID) (float64, bool, error) {
	t := C.roche_next_join(db.raw, idToC(a), idToC(b))
	if t < 0 {
		return -1, false, nil
	}
