...
suite "store persistence":
  test "persistent data dirs are locked across processes":
    let dir = createTempDir("orbelias-store", "lock")
  ...
  test "Particle vec は E レコードで復元される":
    let dir = createTempDir("orbelias-store", "vec")
    var st = openStore(dir)
  ...
  test "payload codec survives WAL replay while legacy records default to raw":
    let dir = createTempDir("orbelias-store", "payload-codec")
    var st = openStore(dir)
    st.upsert Particle(parent: 12'u64, seq: 0'u32, period: 60.0, head: 0.0,
                       tWrite: 1.0, payload: "(object (name OrbeliasDB))",
                       codec: pcNif)
    st.close()
    ...
    let legacyDir = createTempDir("orbelias-store", "legacy-payload-codec")
    writeFile(legacyDir / "orbelias.log",
  ...
  test "stellar map blobs are validated before write and replay":
    let dir = createTempDir("orbelias-store", "stellar-map-validate")
    var st = openStore(dir)
    ...
    let legacyDir = createTempDir("orbelias-store", "stellar-map-bad-replay")
    let malformed = """{"stellar":"commerce/order/A-001","members":[123]}"""
    writeFile(legacyDir / "orbelias.log", "SM " & $malformed.len & "\n" & malformed & "\n")
    expect IOError:
      discard openStore(legacyDir)
    removeDir(legacyDir)

  test "time orbit profile survives WAL replay and compact":
    let dir = createTempDir("orbelias-store", "time-orbit")
    var st = openStore(dir)
  ...
  when declared(poisonWritesForTest):
    test "poisoned write path rejects later persistent mutations":
      let dir = createTempDir("orbelias-store", "poison")
  ...
  test "new WAL records have magic header and checksums":
    let dir = createTempDir("orbelias-store", "wal-v2")
    var st = openStore(dir)
    ...
    let raw = readFile(dir / "orbelias.log")
    check raw.startsWith("!ORBELIASDB-WAL 2\n@ ")
    check raw.contains("\nP 14 0 ")
  ...
  test "versioned WAL torn tail repairs to last checked record":
    let dir = createTempDir("orbelias-store", "wal-v2-tail")
    var st = openStore(dir)
  ...
  test "Forwarder は F レコードで復元される":
    let dir = createTempDir("orbelias-store", "fwd")
    var st = openStore(dir)
  ...
  test "Warp job snapshot は WJ レコードで復元される":
    let dir = createTempDir("orbelias-store", "warp")
    var st = openStore(dir)
  ...
  test "Universe sync event は UJ/UA/UD レコードで復元される":
    let dir = createTempDir("orbelias-store", "universe-sync")
    var st = openStore(dir)
  ...
  test "Universe sync sequence は prune 後も UQ レコードで巻き戻らない":
    let dir = createTempDir("orbelias-store", "universe-seq")
    var st = openStore(dir)
  ...
  test "Universe sync applied dedup set can be pruned with WAL replay":
    let dir = createTempDir("orbelias-store", "universe-applied-prune")
    var st = openStore(dir)
  ...
  test "Universe sync event は store transaction commit まで可視化されない":
    let dir = createTempDir("orbelias-store", "universe-tx")
    var st = openStore(dir)
    var tx = st.beginTxn()
  ...
  test "transaction commit は atomic に復元される":
    let dir = createTempDir("orbelias-store", "tx")
    var st = openStore(dir)
    let tx = st.beginTxn()
  ...
  test "strong durability は単発 write と transaction を再open後も復元する":
    let dir = createTempDir("orbelias-store", "strong")
    var st = openStore(dir, durability = durStrong)
  ...
  test "commit marker のない transaction は replay で無視される":
    let dir = createTempDir("orbelias-store", "tx-partial")
    writeFile(dir / "orbelias.log",
              "T 7\n" &
  ...
  test "WAL 末尾の不完全レコードは最後の完全レコードまで repair される":
    let dir = createTempDir("orbelias-store", "torn-tail")
    ...
    check readFile(dir / "orbelias.log") == good
  ...
  test "WAL 末尾の不正な長さ指定は repair される":
    let dir = createTempDir("orbelias-store", "bad-len-tail")
    let good = "P 2 0 60.0 0.0 1.0 5\nhello\n"
    writeFile(dir / "orbelias.log",
              good &
  ...
  test "WAL 末尾の不正な vector 次元は repair される":
    let dir = createTempDir("orbelias-store", "bad-vec-tail")
    let good = "P 2 0 60.0 0.0 1.0 5\nhello\n"
    writeFile(dir / "orbelias.log",
              good &
              "E 2 0 -1\n")
  ...
  test "WAL 中間破損は tail repair せず起動を拒否する":
    let dir = createTempDir("orbelias-store", "mid-corrupt")
    let wal = "P 2 0 60.0 0.0 1.0 5\nhello\n" &
              "P 2 1 60.0 0.0 2.0 -1 0\n" &
              "P 2 2 60.0 0.0 3.0 5 0\nlater\n"
    writeFile(dir / "orbelias.log", wal)

    expect IOError:
      discard openStore(dir)
    check readFile(dir / "orbelias.log") == wal
    removeDir(dir)

  test "commit marker が torn tail の transaction は repair 後も適用されない":
    let dir = createTempDir("orbelias-store", "tx-torn-tail")
  ...
  test "cluster transaction intent は applied まで保持される":
    let dir = createTempDir("orbelias-store", "cluster-tx")
    var st = openStore(dir)
  ...
  test "compact 中断で tmp だけ残った場合は tmp を正規 WAL として復旧する":
    let dir = createTempDir("orbelias-store", "compact-tmp")
    writeFile(dir / "orbelias.log.compact",
              "G 11\nrecover-tmp\n" &
    ...
    check fileExists(dir / "orbelias.log")
    check not fileExists(dir / "orbelias.log.compact")
    st.close()
    removeDir(dir)

  test "compact 中断で正規 WAL と tmp が両方ある場合は正規 WAL を優先する":
    let dir = createTempDir("orbelias-store", "compact-log-tmp")
    writeFile(dir / "orbelias.log",
  ...
  test "compact 中断で bak だけ残った場合は bak を正規 WAL として復旧する":
    let dir = createTempDir("orbelias-store", "compact-bak")
    writeFile(dir / "orbelias.log.bak",
    ...
    check fileExists(dir / "orbelias.log")
    check not fileExists(dir / "orbelias.log.bak")
    st.close()
    removeDir(dir)

  test "compact は生存レコードだけで WAL を再構築する":
    let dir = createTempDir("orbelias-store", "compact")
    var st = openStore(dir)
  ...
  test "bare delete replay keeps itemsByRing consistent":
    let dir = createTempDir("orbelias-store", "delete-replay-index")
    var st = openStore(dir)
  ...
  test "locality report は interleaved WAL と compact 後の ring grouping を測る":
    let dir = createTempDir("orbelias-store", "locality")
    var st = openStore(dir)
  ...
  test "locality report は上書き済み particle record を dead として数える":
    let dir = createTempDir("orbelias-store", "locality-dead")
    var st = openStore(dir)
  ...
  test "locality report matrix covers delete and backfill fragmentation":
    let dir = createTempDir("orbelias-store", "locality-matrix")
    var st = openStore(dir)
  ...
  test "strong durability の compact 後も WAL は復元できる":
    let dir = createTempDir("orbelias-store", "strong-compact")
    var st = openStore(dir, durability = durStrong)
  ...
  test "backup/restore は compact 済み WAL として別 dir に復元できる":
    let dir = createTempDir("orbelias-store", "backup-src")
    let backupDir = createTempDir("orbelias-store", "backup")
    let restoredDir = createTempDir("orbelias-store", "restore")
    var st = openStore(dir)
    ...
    check not fileExists(backupDir / "orbelias.log.tmp")
    ...
    check not fileExists(restoredDir / "orbelias.log.restore")
  ...
  test "壊れた plain backup は restore 前に拒否され target を壊さない":
    let srcDir = createTempDir("orbelias-store", "backup-corrupt-src")
    let backupDir = createTempDir("orbelias-store", "backup-corrupt")
    let targetDir = createTempDir("orbelias-store", "backup-corrupt-target")
    ...
    createDir(targetDir)
    writeFile(targetDir / "orbelias.log",
              "P 9 0 60.0 0.0 1.0 6 0\nstable\n")
    writeFile(backupDir / "orbelias.log",
              readFile(backupDir / "orbelias.log") &
  ...
  test "encrypted backup/restore は passphrase が一致すると復元できる":
    let dir = createTempDir("orbelias-store", "enc-backup-src")
    let backupDir = createTempDir("orbelias-store", "enc-backup")
    let restoredDir = createTempDir("orbelias-store", "enc-restore")
    var st = openStore(dir)
    ...
    check fileExists(backupDir / "orbelias.backup")
    check not fileExists(backupDir / "orbelias.verify.tmp")
    check not fileExists(backupDir / "orbelias.log.tmp")
    check not readFile(backupDir / "orbelias.backup").contains("secret")
    st.close()
  ...
  test "壊れた encrypted backup は restore 前に拒否され target を壊さない":
    let backupDir = createTempDir("orbelias-store", "enc-corrupt-backup")
    let targetDir = createTempDir("orbelias-store", "enc-corrupt-target")

    writeFile(backupDir / "orbelias.backup", "not-a-orbeliasdb-encrypted-backup")
    createDir(targetDir)
    writeFile(targetDir / "orbelias.log",
  ...
  test "galaxy は data dir に固定され、違う galaxy では開けない":
    let dir = createTempDir("orbelias-store", "galaxy")
    var st = openStore(dir)
