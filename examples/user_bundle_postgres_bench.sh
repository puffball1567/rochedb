...
PG_BIN="${PG_BIN:-}"
TMP_ROOT="${TMPDIR:-/tmp}/koutendb-user-bundle-bench-$$"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
KOUTEN_DATA="$TMP_ROOT/kouten"
PGDATA="$TMP_ROOT/pgdata"
PGLOG="$TMP_ROOT/postgres.log"
KOUTEN_BIN="$ROOT/bin/user_bundle_bench"
...
cleanup() {
  ...
  if [[ "${KEEP_KOUTEN_USER_BUNDLE:-0}" != "1" ]]; then
    rm -rf "$TMP_ROOT"
  else
...
echo "[user-bundle-bench] build KoutenDB benchmark"
nim c -d:release --nimcache:/tmp/nimcache_kouten_user_bundle_bench \
  -o:"$KOUTEN_BIN" examples/user_bundle_bench.nim >/dev/null

echo "[user-bundle-bench] run KoutenDB bundle benchmark"
...
echo "[user-bundle-bench] init temporary PostgreSQL cluster on $PGHOST:$PGPORT"
...
echo "[user-bundle-bench] load PostgreSQL bundle tables"
...
FROM users u CROSS JOIN generate_series(0, 2) AS n;
...
FROM users u CROSS JOIN generate_series(0, 9) AS n;
...
SELECT payload FROM user_addresses WHERE user_id = '$TARGET_ID' ORDER BY n;
SELECT payload FROM user_careers WHERE user_id = '$TARGET_ID' ORDER BY n;
...
SELECT jsonb_build_object(
  ...
  'addresses', (SELECT jsonb_agg(payload ORDER BY n) FROM user_addresses WHERE user_id = '$TARGET_ID'),
  'career', (SELECT jsonb_agg(payload ORDER BY n) FROM user_careers WHERE user_id = '$TARGET_ID'),
  'preferences', (SELECT payload FROM user_preferences WHERE user_id = '$TARGET_ID'),
  'orders', (SELECT jsonb_agg(payload ORDER BY n DESC) FROM user_orders WHERE user_id = '$TARGET_ID')
);
SQL
...
kouten_set="$(metric bundleSetLatencyUs "$kouten_out")"
kouten_set_record="$(metric bundleSetUsPerRecord "$kouten_out")"
kouten_pack="$(metric bundlePackLatencyUs "$kouten_out")"
kouten_pack_records="$(metric bundlePackRecords "$kouten_out")"
kouten_read="$(metric bundleReadLatencyUs "$kouten_out")"
kouten_count="$(metric bundleReadCount "$kouten_out")"
kouten_rings="$(metric bundleReadRings "$kouten_out")"
kouten_records="$(metric bundleLogicalRecords "$kouten_out")"

echo
echo "== KoutenDB vs PostgreSQL user bundle benchmark =="
echo "users: $N"
echo "logical records: $kouten_records"
echo "target: $TARGET_ID"
echo "reads: $READS"
echo
echo "| system | layout/query | setup or load note | read latency us |"
echo "| --- | --- | --- | ---: |"
echo "| KoutenDB | users/<id>/* stellar depth read | set ${kouten_set} us, ${kouten_set_record} us/record, pack ${kouten_pack} us (${kouten_pack_records} records) | $kouten_read |"
echo "| PostgreSQL | 5 indexed SELECT statements | tables analyzed after load | $pg_multi_us |"
echo "| PostgreSQL | one JSON aggregate query over indexed tables | tables analyzed after load | $pg_json_us |"
echo
echo "This benchmark compares a user-detail bundle, not a single primary-key lookup."
echo "KoutenDB stores the bundle as coordinate-local rings; PostgreSQL stores the same logical data in normalized indexed tables."
