...
WORK="${TMPDIR:-/tmp}/orbeliasdb-universe-sync-demo-$$"
...
nim c -d:release --nimcache:/tmp/nimcache_orbelias_universe_sync_cli \
...
bin/orbeliascli universe-export --data="$CLI_SOURCE" --out="$WORK/pending.jsonl"
echo "exported lines: $(wc -l < "$WORK/pending.jsonl")"
bin/orbeliascli universe-sync --data="$CLI_SOURCE" --target-data="$CLI_TARGET" --prune-acked
bin/orbeliascli universe-export --data="$CLI_SOURCE" --out="$WORK/after.jsonl"
