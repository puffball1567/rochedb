## Physical locality demo for OrbeliasDB's WAL layout.
...
proc measureReadUs(db: OrbeliasDb, ring: string, iters: int): float =
  if iters <= 0:
    return 0.0
  let start = cpuTime()
  var consumed = 0
  for _ in 0 ..< iters:
    let page = db.readRing(ring, OrbeliasReadOptions(
      filter: newJObject(),
      limit: 100,
      sortField: "id",
      sortDirection: rsAsc))
...
proc logicalSignature(db: OrbeliasDb, ring: string): seq[string] =
...
when isMainModule:
  ...
  let dataDir =
    ...
    else: createTempDir("orbeliasdb", "locality-layout-demo")
  ...
  try:
    var ids: seq[seq[OrbeliasId]] = newSeq[seq[OrbeliasId]](rings)
