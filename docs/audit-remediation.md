...
This document tracks OrbeliasDB's response to the 2026-07-16 external audit notes.
...
| Decision | A direct patch would change OrbeliasDB's model; decide the intended behavior first. |
| Planned | Accepted as future work, not implemented in this branch. |
| Not aligned | Intentionally not adopted because it conflicts with OrbeliasDB's concept or current scope. |
...
| R4 | Escaping `Defect` could return `ORBELIAS_OK` through the C ABI. | Done | `config.nims` enables `--panics:on`; C ABI validates node count and oversized lengths; C handle safety tests were expanded. Covered by `examples/cabi_contract.c`. |
| R5 | WAL lacked checksum/versioning and unknown records could desynchronize replay into payload bytes. | Done | New WAL files use `!ORBELIASDB-WAL 2` plus per-record length and CRC32 wrappers. Versioned checksum mismatch refuses open; versioned torn tails repair to the last checked record. Legacy pre-v1.0 WAL remains readable. Covered by new `tests/tstore.nim` WAL v2 cases. |
...
| R7 | Unusable auth configs such as `--secret-key` without `--user` failed open. | Done | `orbeliasd` rejects unusable user/password/secret-key combinations at startup. Covered by `scripts/cluster_authz_smoke.sh`. |
...
| Durability | fsync failure can leave memory ahead of disk. | Done | Store now marks the persistent write path as poisoned after flush/fsync failure and rejects later mutations on that handle. `-d:orbeliasTestFailpoints tests/tstore.nim` covers the fail-closed contract. |
...
| C ABI | `last_error` pointer lifetime was unclear. | Done | `include/orbeliasdb.h` documents that the pointer is valid only until the next OrbeliasDB C ABI call on the same thread. |
...
| C ABI | `orbelias_init` was not idempotent. | Done | `orbelias_init()` is guarded; contract test calls it twice. |
| C ABI | Thread-safety contract was not documented. | Done | `include/orbeliasdb.h` and `docs/driver-installation.md` document `orbelias_init` idempotency, `last_error` lifetime, handle sharing, and `orbelias_close` synchronization requirements. |
...
| Server | Credentials are passed as CLI arguments in examples. | Done | Server and CLI now accept `--password-file`, `--auth-token-file`, `--secret-key-file`, and `ORBELIAS_*` env fallbacks. Docs prefer file/env paths outside local smoke tests; `cluster_authz_smoke` covers file/env auth. |
...
| Cross-version migration needs a stable boundary while WAL can evolve before v1.0. | Done | `orbeliasdb.dump.v1` JSONL is documented as the migration boundary; `importJsonl` recognizes OrbeliasDB dump files and preserves ring, payload, vector, and codec metadata. Covered by API and CLI round-trip tests. |
...
| Item | Reason |
|---|---|
| Make Raft/consensus the default cluster write path. | This conflicts with OrbeliasDB's current direction: reduce global coordination and use ring/galaxy/universe boundaries instead. Coordinator redundancy and recovery are still valid goals, but not by defaulting every write to consensus. |
...
| Make secondary indexes the primary model. | OrbeliasDB's primary access model is ring/stellar locality. Secondary mechanisms should remain hints, projections, or lookup maps that point back into ring-local reads. |
| Make cooperative locks mandatory for normal writes. | That would make the normal NoSQL path heavier and less OrbeliasDB-like. Locks should remain opt-in unless a high-integrity workflow asks for them. |

## Verification Snapshot

The current hardening branch has been checked with:

- `nim check src/orbelias/store.nim`
- `nim c -r --nimcache:/tmp/nimcache_orbelias_tstore tests/tstore.nim`
- `nim c -r --nimcache:/tmp/nimcache_orbelias_tapi tests/tapi.nim`
