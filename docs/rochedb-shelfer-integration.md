# RocheDB x Shelfer Integration

RocheDB and Shelfer address the same cost problem at different layers. RocheDB
...
RocheDB reduces the candidate set before data reaches an LLM or reranker through
...
Therefore RocheDB should not become Shelfer's internal database. The default
integration shape is that Shelfer sees RocheDB as a RAG source, document source,
or retrieval source.

```text
corpus / events / documents
  -> RocheDB galaxy / ring / vector backend
  -> RocheDB retrieval envelope
  -> Shelfer RAG source adapter
  -> Shelfer Delivery / policy / metrics / audit
  -> LLM or allowlisted MCP workers
  -> Shelfer utility feedback
  -> RocheDB routing and ring policy tuning
```
...
| --- | --- |
| RocheDB | document / vector storage, ring scoped retrieval, galaxy isolation, candidate reduction, payload projection, auth boundary |
...
RocheDB should not expose Shelfer-specific runtime internals in its public core.
Shelfer should not depend on RocheDB's orbital internals. The shared contract is
a small retrieval envelope plus metrics.

## Retrieval Envelope Contract

Shelfer is expected to release after RocheDB, so RocheDB should stabilize an
...
A RocheDB-backed source returns an envelope shaped like this:

```json
{
  "schema": "rochedb.retrieval.v1",
  "version": 1,
  "source": {
    "provider": "rochedb",
    "galaxy": "tenant-a",
  ...
  "chunks": [
    {
      ...
      "estimatedTokens": 120,
      "ring": "docs/security",
      "sourceUri": "rochedb://tenant-a/docs/security/0000000000000001:00000002"
    }
  ],
  "stats": {
...
without making Shelfer a hard dependency of RocheDB.
...
| `source.provider` | Usually `rochedb`; adapters may namespace derived providers |
...
- RocheDB core may add optional fields to v1 envelopes.
- RocheDB must not remove or rename required v1 fields without a new schema.
...
RocheDB should expose query execution as a plan, similar in spirit to SQL
...
- Planner selection is deterministic heuristic ranking inside RocheDB. Model
...
| `scope` | `ssTight`, `ssNear`, `ssWide`, `ssAll` | How widely RocheDB may search across related rings |
| `depth` | `sdShallow`, `sdNormal`, `sdDeep`, `sdVeryDeep` | How deeply RocheDB may descend through ring hierarchy |
...
RocheDB exposes named tuning profiles for this purpose:
...
| RDB concept | RocheDB equivalent |
| --- | --- |
...
| workload advisor | external agent using atlas and stats, not RocheDB read path |
...
RocheDB already exposes `RetrieveStats` fields that map naturally to Shelfer's
RAG/resource utility and context investment metrics.

| RocheDB field | Shelfer usage |
| --- | --- |
| `chunks[].id` | `selectedResources`, `usedItemIds`, `ignoredItemIds` |
| `source.galaxy` | `resourceProvider = "rochedb:<galaxy>"` or tenant label |
...
which chunks were used, and which chunks were ignored. RocheDB should use that
feedback to tune ring policies and default budgets, not to silently change
authorization or runtime policy.

## Safety Boundary

RocheDB retrieval results are data cargo. Shelfer should treat them as
untrusted RAG content until runtime policy accepts them.

- Auth, secret keys, and galaxy isolation remain RocheDB-side access controls.
- Shelfer applies worker allowlists, labels, Delivery policy, and audit.
- Prompt/content inspection can run before RocheDB chunks enter an LLM context.
...
1. Keep RocheDB core independent and emit the stable retrieval envelope.
