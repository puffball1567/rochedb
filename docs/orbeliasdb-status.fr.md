# Statut / feuille de route OrbeliasDB

Ce document est une traduction de reference. Le fichier anglais
[orbeliasdb-status.md](./orbeliasdb-status.md) fait autorite; cette traduction peut
etre en retard.
...
| FAISS vector backend | PoC | Dynamic bridge via `liborbelias_faiss.so`; default fetch tag is FAISS `v1.14.3`; exact commit pinning is optional via `ORBELIAS_FAISS_COMMIT` |
...
| Node.js / TypeScript / Bun | Partially published | npm `orbeliasdb` v0.1.3. Bun remains experimental |
| Rust | Published | crates.io `orbeliasdb` v0.1.3; see the English canonical document for links |
...
| Package publishing | Partial | `nimble install orbeliasdb`, `cargo add orbeliasdb`, `npm install orbeliasdb`, `composer require orbeliasdb/orbeliasdb`, and `python3 -m pip install orbeliasdb` are available. Other registries remain future work |
...
| Item | Status | Notes |
|---|---|---|
| Username/password auth | Done | orbeliasd and driver path |
