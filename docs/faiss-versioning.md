...
KoutenDB does not vendor the FAISS source tree. The repository keeps only:

- `scripts/fetch_faiss.sh`
- `scripts/build_faiss_bridge.sh`
- `third_party/faiss.version`
- the KoutenDB bridge source in `src/kouten/faiss_bridge.cpp`

The FAISS checkout itself lives at `third_party/faiss/` and is ignored by Git.
The built bridge `lib/libkouten_faiss.so` is also ignored.
...
By default, KoutenDB fetches the configured FAISS tag without enforcing a commit
pin:
...
update, users can move to another tag without waiting for KoutenDB to publish a
new default.

## Installing a Specific Tag

Use `KOUTEN_FAISS_VERSION`:

```sh
KOUTEN_FAISS_VERSION=v1.14.2 scripts/fetch_faiss.sh
scripts/build_faiss_bridge.sh
kouten doctor
```
...
Use both `KOUTEN_FAISS_VERSION` and `KOUTEN_FAISS_COMMIT`.

```sh
KOUTEN_FAISS_VERSION=v1.14.3 \
KOUTEN_FAISS_COMMIT=0ca9df4792b173d573044ee14ca0704780176e82 \
scripts/fetch_faiss.sh
```

When `KOUTEN_FAISS_COMMIT` is set, `scripts/fetch_faiss.sh` fails if the fetched
...
2. Fetch it:
   ...
   ```sh
   KOUTEN_FAISS_VERSION=vX.Y.Z scripts/fetch_faiss.sh
   ```
...
5. For reproducible builds, rerun with `KOUTEN_FAISS_COMMIT=<that commit>`.

Downgrades use the same process with an older tag.

## KoutenDB Maintainer Pin Updates

When KoutenDB maintainers update the recommended default:

1. Choose the new default FAISS tag.
2. Test it with `scripts/fetch_faiss.sh`, `scripts/build_faiss_bridge.sh`,
   `kouten doctor`, and the release smoke suite.
...
KoutenDB maintainers should then test the patched version and update the
