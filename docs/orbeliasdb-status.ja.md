# OrbeliasDB 状態 / ロードマップ

この文書は参考翻訳です。英語版 [orbeliasdb-status.md](./orbeliasdb-status.md) が正本であり、
この翻訳は遅れる可能性があります。
...
| Atlas / ring map | 完了 | `atlas()` と `orbelias atlas` |
| Retrieval tuning profile | 完了 | amount / scope / depth |
| FAISS vector backend | PoC | `liborbelias_faiss.so` dynamic bridge。default fetch tag は FAISS `v1.14.3`; exact commit pinning は `ORBELIAS_FAISS_COMMIT` で optional |
...
| 機能 | 状態 | メモ |
|---|---|---|
| Static cluster | 完了 | `orbeliasd --id --peers` |
...
| Node.js / TypeScript / Bun | 一部公開済み | npm `orbeliasdb` v0.1.3。Bun は実験的 |
| Rust | 公開済み | crates.io `orbeliasdb` v0.1.3。詳細は英語正本を参照 |
...
| Driver discovery CLI | 完了 | `orbelias driver list/info/install` が公式 driver metadata と setup command を表示。remote script は実行しない |
| Package publishing | 一部完了 | `nimble install orbeliasdb`, `cargo add orbeliasdb`, `npm install orbeliasdb`, `composer require orbeliasdb/orbeliasdb`, `python3 -m pip install orbeliasdb` が利用可能。その他のレジストリは今後 |
...
| 項目 | 状態 | メモ |
|---|---|---|
| Username/password auth | 完了 | orbeliasd and driver path |
