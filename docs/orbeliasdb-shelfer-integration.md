# OrbeliasDB x Shelfer Integration

OrbeliasDB and Shelfer address the same cost problem at different layers. OrbeliasDB
...
OrbeliasDB reduces the candidate set before data reaches an LLM or reranker through
...
Therefore OrbeliasDB should not become Shelfer's internal database. The default
integration shape is that Shelfer sees OrbeliasDB as a RAG source, document source,
or retrieval source.

```text
corpus / events / documents
  -> OrbeliasDB galaxy / ring / vector backend
  -> OrbeliasDB retrieval envelope
  -> Shelfer RAG source adapter
  -> Shelfer Delivery / policy / metrics / audit
  -> LLM or allowlisted MCP workers
  -> Shelfer utility feedback
  -> OrbeliasDB routing and ring policy tuning
```
...
| --- | --- |
| OrbeliasDB | document / vector storage, ring scoped retrieval, galaxy isolation, candidate reduction, payload projection, auth boundary |
...
OrbeliasDB should not expose Shelfer-specific runtime internals in its public core.
Shelfer should not depend on OrbeliasDB's orbital internals. The shared contract is
a small retrieval envelope plus metrics.

## Retrieval Envelope Contract

Shelfer is expected to release after OrbeliasDB, so OrbeliasDB should stabilize an
...
A OrbeliasDB-backed source returns an envelope shaped like this:

```json
{
  "schema": "orbeliasdb.retrieval.v1",
  "version": 1,
  "source": {
    "provider": "orbeliasdb",
    "galaxy": "tenant-a",
  ...
  "chunks": [
    {
      ...
      "estimatedTokens": 120,
      "ring": "docs/security",
      "sourceUri": "orbeliasdb://tenant-a/docs/security/0000000000000001:00000002"
    }
  ],
  "stats": {
...
without making Shelfer a hard dependency of OrbeliasDB.
...
| `source.provider` | Usually `orbeliasdb`; adapters may namespace derived providers |
...
- OrbeliasDB core may add optional fields to v1 envelopes.
- OrbeliasDB must not remove or rename required v1 fields without a new schema.
...
OrbeliasDB should expose query execution as a plan, similar in spirit to SQL
...
- Persistent embedded stores can compact the WAL with `compact` /
  `orbelias compact --data=DIR`, keeping only live records and metadata.
- Persistent embedded stores can create and restore compact WAL backups with
  `backup` / `restoreBackup` or `orbelias backup --data=DIR --backup=DIR` and
  `orbelias restore --backup=DIR --data=DIR`. Restore can use
...
- Persistent embedded stores can export readable JSON Lines dumps with `dump`
  or `orbelias dump --data=DIR --out=FILE`; this is for inspection and
...
- External NoSQL JSON Lines exports can be imported with routing rules:
  ...
  `orbelias import-jsonl --data=DIR --in=FILE --ring-field=tenant`.
  The selected ring is created automatically, so imports can distribute
  existing documents into OrbeliasDB's ring hierarchy during ingestion.
...
- Planner selection is deterministic heuristic ranking inside OrbeliasDB. Model
...
| `scope` | `ssTight`, `ssNear`, `ssWide`, `ssAll` | How widely OrbeliasDB may search across related rings |
| `depth` | `sdShallow`, `sdNormal`, `sdDeep`, `sdVeryDeep` | How deeply OrbeliasDB may descend through ring hierarchy |
...
OrbeliasDB exposes named tuning profiles for this purpose:
...
| RDB concept | OrbeliasDB equivalent |
| --- | --- |
...
| workload advisor | external agent using atlas and stats, not OrbeliasDB read path |
...
OrbeliasDB already exposes `RetrieveStats` fields that map naturally to Shelfer's
RAG/resource utility and context investment metrics.

| OrbeliasDB field | Shelfer usage |
| --- | --- |
| `chunks[].id` | `selectedResources`, `usedItemIds`, `ignoredItemIds` |
| `source.galaxy` | `resourceProvider = "orbeliasdb:<galaxy>"` or tenant label |
...
which chunks were used, and which chunks were ignored. OrbeliasDB should use that
feedback to tune ring policies and default budgets, not to silently change
authorization or runtime policy.

## Safety Boundary

OrbeliasDB retrieval results are data cargo. Shelfer should treat them as
untrusted RAG content until runtime policy accepts them.

- Auth, secret keys, and galaxy isolation remain OrbeliasDB-side access controls.
- Shelfer applies worker allowlists, labels, Delivery policy, and audit.
- Prompt/content inspection can run before OrbeliasDB chunks enter an LLM context.
...
1. Keep OrbeliasDB core independent and emit the stable retrieval envelope.
2. Add a small OrbeliasDB RAG adapter for Shelfer as a separate module or plugin.
3. Map OrbeliasDB `RetrieveStats` into Shelfer RAG/resource utility records.
4. Add a feedback loop that recommends ring budgets and source routing.
5. Measure quality-fixed reductions with `orbelias rag-bench` and Shelfer
