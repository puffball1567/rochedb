...
- Japanese: [orbeliasdb-status.ja.md](./orbeliasdb-status.ja.md)
- German: [orbeliasdb-status.de.md](./orbeliasdb-status.de.md)
- French: [orbeliasdb-status.fr.md](./orbeliasdb-status.fr.md)
- Chinese: [orbeliasdb-status.zh.md](./orbeliasdb-status.zh.md)
- Korean: [orbeliasdb-status.ko.md](./orbeliasdb-status.ko.md)
...
| Payload codecs | PoC | Per-record `raw` / `json` / `nif` / `bif` metadata survives WAL, cluster transport, handoff, transactions, Universe sync, and retrieval. NIF/BIF encoding/decoding remains outside the core; optional adapter: [`orbeliasdb-nif`](https://github.com/puffball1567/orbeliasdb-nif) backed by [`nifkit`](https://github.com/puffball1567/nifkit) |
...
| Atlas / ring map | Done | `atlas()` and `orbelias atlas` |
| Galaxy/ring description | Done | Atlas map annotations, not payload text |
| Time orbit | PoC | Embedded ring-local 60-bit millisecond orbit for log/event/time-series placement. Includes persisted profiles, `putTime` / `readTime`, and `orbelias time-orbit/time-put/time-get`; cluster profile administration is still pending. Design note: [time-orbit.md](./time-orbit.md) |
| Retrieval tuning profile | Done | amount / scope / depth |
| FAISS vector backend | PoC | Dynamic bridge via `liborbelias_faiss.so`; default fetch tag is FAISS `v1.14.3`, exact commit pinning is optional via `ORBELIAS_FAISS_COMMIT`; `orbelias doctor`, bridge build, `tests/tapi.nim`, and `examples/vector_backend_bench.sh` are verified locally |
| FAISS GPU backend | Not planned for core | OrbeliasDB is designed to reduce the search set before ANN/LLM work. Needing GPU FAISS is treated as a placement or retrieval-profile issue first |
...
| Strong durability / fsync knob | Done | `open(dataDir=..., durability=durStrong)` and `orbeliasd --durability=strong`; store/API tests cover reopen, transaction, compact |
...
| Feature | Status | Notes |
|---|---|---|
| Static cluster | Done | `orbeliasd --id --peers` |
...
| TLS | Done | Standard TLS transport for `orbeliasd` and CLI/client connections when built with `-d:ssl`; `scripts/cluster_tls_smoke.sh` covers authenticated TLS, secret-key transport, JSON put/get, and plain-client rejection |
| Authz / RBAC | PoC | `orbeliasd --allow-ring=prefix[,prefix...]` and `--role=user:password:reader|writer|admin[:prefixes]`; `scripts/cluster_authz_smoke.sh` and `scripts/cluster_rbac_smoke.sh` cover prefix and role matrix behavior |
...
| Bounded server retrieve | Done | `orbeliasd` keeps only the current top candidates up to request budget while scanning local vectors instead of retaining every matching payload before truncation |
...
| JavaScript / TypeScript | Published | npm [`orbeliasdb` v0.1.3](https://www.npmjs.com/package/orbeliasdb); repository [`puffball1567/orbeliasdb-js`](https://github.com/puffball1567/orbeliasdb-js); Node-API C ABI wrapper with TypeScript API |
...
| Rust | Published | crates.io [`orbeliasdb` v0.1.3](https://crates.io/crates/orbeliasdb); repository [`puffball1567/orbeliasdb-rust`](https://github.com/puffball1567/orbeliasdb-rust); C ABI wrapper |
| Python | Published | PyPI [`orbeliasdb` v0.1.3](https://pypi.org/project/orbeliasdb/); repository [`puffball1567/orbeliasdb-python`](https://github.com/puffball1567/orbeliasdb-python); native TCP wire driver |
| Go | Done | C ABI wrapper minimal |
| PHP | Published | Packagist [`orbeliasdb/orbeliasdb` v0.1.2](https://packagist.org/packages/orbeliasdb/orbeliasdb); repository [`puffball1567/orbeliasdb-php`](https://github.com/puffball1567/orbeliasdb-php); FFI / C ABI wrapper with Docker smoke |
...
| C++ | Released | Repository [`puffball1567/orbeliasdb-cpp` v0.1.1](https://github.com/puffball1567/orbeliasdb-cpp); C++17 C ABI wrapper with CMake smoke; Unreal official plugin is separate |
...
| Driver discovery CLI | Done | `orbelias driver list/info/install` prints official driver metadata and setup commands without executing remote scripts |
| Driver compatibility test suite | Partial | `scripts/driver_compat.sh`; Docker-backed PHP / Swift / Kotlin are opt-in and verified |
| Package publishing | Partial | `nimble install orbeliasdb`, `cargo add orbeliasdb`, `npm install orbeliasdb`, `composer require orbeliasdb/orbeliasdb`, and `python3 -m pip install orbeliasdb` are available. NuGet, Maven, Go, SwiftPM, and other registry packages remain future work |
...
| Item | Status | Notes |
|---|---|---|
| Username/password auth | Done | orbeliasd and driver path; user without password fails closed |
...
| Backup encryption | Done | `backupEncrypted` / `restoreEncryptedBackup` and `orbelias backup-encrypted` / `restore-encrypted` use nimsodium secretbox |
...
OrbeliasDB should be able to become a managed service in the same operational
category as hosted cache, document, search, or AI-context databases. Some
managed-service requirements are already expressible through OrbeliasDB concepts:
...
| OpenMetrics / CloudWatch / Datadog adapters | OrbeliasDB exposes key/value metrics, but managed integrations need standard exporters or collectors. |
...
provider-ready managed database. OrbeliasDB should not copy every Redis, RDS, or
ElastiCache mechanism one-to-one; it should provide equivalent operational
outcomes where OrbeliasDB's universe / galaxy / ring model already gives a simpler
