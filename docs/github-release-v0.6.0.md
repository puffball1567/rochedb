...
KoutenDB v0.6.0 is a technical-preview release focused on topology remapping
foundations, locality validation, and easier operational configuration.

Release:

https://github.com/puffball1567/koutendb/releases/tag/v0.6.0
...
claim in this release is narrower: KoutenDB now has more measurable support for
...
KoutenDB's thesis is that meaningful placement should reduce unnecessary reads,
...
Second, topology remapping now has explicit primitives. KoutenDB still does not
...
```sh
kouten --config=kouten.json health

KOUTEN_CONFIG=kouten.json kouten put \
  --ring=docs/japan \
  --payload='{"title":"Hello","status":"draft"}' \
  --codec=json

KOUTEN_CONFIG=kouten.json kouten get \
  --ring=docs/japan \
  --filter='{"status":"draft"}' \
