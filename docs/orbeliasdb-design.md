...
- Concept: [orbeliasdb-concept.md](./orbeliasdb-concept.md)
- Benchmark notes: [orbeliasdb-bench.md](./orbeliasdb-bench.md)
- Feature status: [orbeliasdb-status.md](./orbeliasdb-status.md)
- Shelfer boundary: [orbeliasdb-shelfer-integration.md](./orbeliasdb-shelfer-integration.md)
- Halo capture: [orbeliasdb-halo-capture.md](./orbeliasdb-halo-capture.md)

## 1. Design Goal

OrbeliasDB is a placement-aware document/vector store. It is designed to reduce the
...
policy. The same OrbeliasDB mechanism can host unrelated datasets without making
...
OrbeliasDB exposes a retrieval plan in the envelope. Human-facing knobs are:

| Field | Meaning |
|---|---|
| `amount` | how many useful chunks to return |
| `scope` | how widely OrbeliasDB may search across related rings |
| `depth` | how deeply OrbeliasDB may descend through hierarchy |
...
OrbeliasDB has an exact backend for correctness, tests, and small deployments. The
...
does heavy work. OrbeliasDB should not require GPU FAISS as the default path. The
...
OrbeliasDB is not a mandatory write-time classifier. Expensive inference should
...
Cluster transaction landing is intentionally conservative. OrbeliasDB does not
...
Most OrbeliasDB data should not require a global application order. If related
...
This avoids turning OrbeliasDB into a heavyweight global-order database. Strict
ordering should be opt-in at the ring level.

For rings where order changes the final state, OrbeliasDB should use delayed
apply:
...
The v0.2 implementation starts this as a durable outbox: OrbeliasDB can persist
...
sync model. OrbeliasDB should keep those workloads separate instead of forcing all
...
Cluster mode runs multiple `orbeliasd` nodes with a shared peer list. The client can
...
OrbeliasDB supports username/password style authentication with an additional
...
OrbeliasDB data retrieved for RAG or tools is untrusted content until a host policy
...
OrbeliasDB can reduce application complexity when routes and state scopes map to
rings:
...
For AI systems, OrbeliasDB should be used before expensive reasoning:
...
See [orbeliasdb-bench.md](./orbeliasdb-bench.md).
