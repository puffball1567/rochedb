# OrbeliasDB v0.3.0 Technical Preview

OrbeliasDB v0.3.0 expands the driver-facing and codec-aware read surface. The
notable change is C ABI v2 with `orbelias_read_ring_json`, which gives external
drivers a stable ring-read page shape similar to the CLI `orbelias get --ring=...`
workflow.
...
- C ABI v2 with `orbelias_read_ring_json` for JSON filters, optional projection,
