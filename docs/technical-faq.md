...
This document answers the questions OrbeliasDB is most likely to receive from
database engineers who are seeing the project for the first time.

OrbeliasDB is still a technical preview. The goal of this FAQ is not to overstate
...
A OrbeliasDB `ring` has a partition-like effect because it narrows the candidate
...
The difference is that OrbeliasDB treats the ring as a first-class coordinate used
...
collection. In OrbeliasDB, placement is part of the public data model. The
...
OrbeliasDB cannot magically recover good locality. The benefit appears when ring
...
OrbeliasDB's current secondary access direction is deliberately conservative:
...
This is an intentional tradeoff. OrbeliasDB is trying to make locality the primary
...
reason OrbeliasDB reduces scanned records in the current benchmarks.
...
The orbital part matters for a different reason. OrbeliasDB stores compact
...
topology/remapping foundation: OrbeliasDB can now represent the safer layout, test
...
ring, then OrbeliasDB can avoid scanning most records before filtering, ranking,
projection, or LLM/RAG context construction.

That is not the same claim as "OrbeliasDB is always faster than every database."
It is a narrower claim:

> When the application has meaningful locality, OrbeliasDB can make that locality
...
interleaved across rings, the WAL is also interleaved. OrbeliasDB exposes locality
...
locality. OrbeliasDB's current answer is:
...
## What is OrbeliasDB best suited for today?

OrbeliasDB is most interesting when the application already has meaningful locality:
...
Use this when describing OrbeliasDB:

```text
OrbeliasDB is a placement-aware NoSQL document/vector store that makes
...
```text
OrbeliasDB is a universal replacement for PostgreSQL, Redis, MongoDB, or vector
databases.
```
