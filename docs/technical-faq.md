...
This document answers the questions KoutenDB is most likely to receive from
database engineers who are seeing the project for the first time.

KoutenDB is still a technical preview. The goal of this FAQ is not to overstate
...
A KoutenDB `ring` has a partition-like effect because it narrows the candidate
...
The difference is that KoutenDB treats the ring as a first-class coordinate used
...
collection. In KoutenDB, placement is part of the public data model. The
...
KoutenDB cannot magically recover good locality. The benefit appears when ring
...
KoutenDB's current secondary access direction is deliberately conservative:
...
This is an intentional tradeoff. KoutenDB is trying to make locality the primary
...
reason KoutenDB reduces scanned records in the current benchmarks.
...
The orbital part matters for a different reason. KoutenDB stores compact
...
topology/remapping foundation: KoutenDB can now represent the safer layout, test
...
ring, then KoutenDB can avoid scanning most records before filtering, ranking,
projection, or LLM/RAG context construction.

That is not the same claim as "KoutenDB is always faster than every database."
It is a narrower claim:

> When the application has meaningful locality, KoutenDB can make that locality
...
interleaved across rings, the WAL is also interleaved. KoutenDB exposes locality
...
locality. KoutenDB's current answer is:
...
## What is KoutenDB best suited for today?

KoutenDB is most interesting when the application already has meaningful locality:
...
Use this when describing KoutenDB:

```text
KoutenDB is a placement-aware NoSQL document/vector store that makes
...
```text
KoutenDB is a universal replacement for PostgreSQL, Redis, MongoDB, or vector
databases.
```
