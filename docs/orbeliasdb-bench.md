...
- Redis measurements are local smoke tests for simple read and batch-read
  latency.
- Working-set, memory-pressure, and RAG-style measurements test OrbeliasDB's main
...
- Environment: AMD Ryzen 5 5600H / Linux 6.8 / Nim 2.2.10 `-d:danger` / gcc
  `-O2` / `--mm:arc`
- Reproduction: `bin/orbeliasbench` (`src/orbeliasbench.nim`) and `bin/cbench`
  (`examples/cbench.c`)
...
lookup. The point is not raw speed alone. At similar cost, OrbeliasDB also answers
...
Interpretation: the 2x to 9x overhead is the cost of OrbeliasDB's orbit-aware ID
...
These operations are hard to express in a directory-only design. OrbeliasDB can use
...
| Operation | ns/op | Notes |
|---|---:|---|
| `orbelias_put` | 362.5 | Nim API plus roughly 98 ns for FFI and payload copy |
| `orbelias_get` | 194.3 | Includes duplicate buffer, NUL terminator, and `orbelias_free`; access pattern differs from random get |
| `orbelias_locate`, future time | 77.7 |  |
...
- OrbeliasDB setup: three `orbeliasd` nodes, persistence enabled, single client,
  persistent TCP connection, 100-byte payload, `n=10000`
- Reproduction: start three `orbeliasd` processes with `--id=k --peers=...
  --data=...`, then run `orbelias bench --peers=... --n=10000`
- Reproduction helper: `N=10000 examples/postgres_bench.sh` starts a temporary
  three-node OrbeliasDB cluster and a temporary local PostgreSQL cluster, then runs
  both benchmark shapes. It requires `initdb`, `pg_ctl`, `psql`, and `pgbench`
  from a local PostgreSQL installation. Both OrbeliasDB and PostgreSQL use fresh
...
- Docker reproduction helper: `N=10000 examples/postgres_docker_bench.sh`
  starts three OrbeliasDB containers and one PostgreSQL container on the same
...
### OrbeliasDB Measurements

| Operation | us/op | Notes |
|---|---:|---|
| single-key read | 46.8 | Three `orbeliasd` nodes, persistence enabled |
| single-row write | 48.8 | Three `orbeliasd` nodes, persistence enabled |
...
that OrbeliasDB's network KV path is in the same latency class as PostgreSQL
primary-key access and is ahead under these local conditions: PostgreSQL SELECT
was about 1.45x slower than OrbeliasDB read, and PostgreSQL
`synchronous_commit=off` write was about 1.64x slower than OrbeliasDB write in this
run. OrbeliasDB's durability mode in this comparison was closer to
`synchronous_commit=off`.
OrbeliasDB now has `durStrong` / `--durability=strong`, but that path was not part
of the 2026-07-15 PostgreSQL comparison.

## Docker-Docker PostgreSQL Reference

- Date: 2026-07-15
- Environment: same host, Docker `overlay2`, OrbeliasDB image built from
  `examples/compose/Dockerfile`, PostgreSQL image `postgres:14`
- Setup: three OrbeliasDB containers and one PostgreSQL container on the same
  Docker network, single client, 100-byte payload, `n=10000`
- Data placement: OrbeliasDB and PostgreSQL data directories are bind-mounted from
...
### OrbeliasDB Docker Measurements
...
SELECT was about 1.7x slower than OrbeliasDB read, and PostgreSQL
`synchronous_commit=off` write was about 1.4x slower than OrbeliasDB write.
...
- Redis: local `/usr/bin/redis-server`, Redis 6.0.16, endpoint
  `127.0.0.1:6379`
- OrbeliasDB TCP: local one-node `orbeliasd`, endpoint `127.0.0.1:17301`,
  persistence disabled, persistent TCP connection
- Conditions: 100-byte payload, `n=1000`, single client, Redis pipeline batch
  size 256
- Reproduction: start one local `orbeliasd`, then run
  `orbelias redis-bench --n=1000 --payload-bytes=100 --redis=127.0.0.1:6379
  --peers=127.0.0.1:17301`
- Local reproduction helper: `N=1000 examples/redis_local_bench.sh` starts one
  local `orbeliasd` with a fresh temporary data directory and compares it with an
  existing local Redis endpoint. Redis keys use a unique
  `orbeliasdb:bench:<timestamp>:` prefix and are deleted before exit.
- Docker reproduction helper: `N=1000 examples/redis_docker_bench.sh` builds a
  OrbeliasDB Docker image, starts Redis and OrbeliasDB on the same Docker network, and
...
- Purpose: smoke-test whether OrbeliasDB simple read and batch read are in the same
...
| Operation | us/op | Interpretation |
|---|---:|---|
| OrbeliasDB embedded get | 0.04 | In-process hot path inside the benchmark container |
| OrbeliasDB TCP get | 55.78 | One request / one response across the Docker network |
| OrbeliasDB TCP BGET | 1.71 | Batch read; comparable axis to Redis pipeline |
| Redis TCP GET | 48.74 | Redis 7 container, non-pipelined |
| Redis pipeline GET | 2.06 | Batch size 256 |

Interpretation: OrbeliasDB TCP get is in the same latency class as non-pipelined
Redis GET, but local Redis was slightly faster for single GET. In this smoke
test, OrbeliasDB TCP BGET was about 2.4x faster than Redis pipeline GET. In the
Docker-Docker run, OrbeliasDB TCP BGET was about 1.2x faster than Redis pipeline
GET. This is
not a claim that OrbeliasDB is always faster than Redis.
Payload size, batch size, Redis configuration, network mode, and data size need
broader measurement.

The important point for OrbeliasDB is narrower: it is not merely reducing
...
- Reproduction: `orbelias working-set-bench --n=10000 --rings=100
...
showing that OrbeliasDB is not just filtering after retrieval.
...
- Environment: same machine, AMD Ryzen 5 5600H / Linux 6.8 / Nim 2.2.10
  `-d:release`, embedded mode, persistence disabled
- Reproduction: `orbelias memory-pressure-bench --n=100000 --rings=100
...
preprocessing would need to keep as candidates. OrbeliasDB does not manufacture
...
- Environment: same machine, AMD Ryzen 5 5600H / Linux 6.8 / Nim 2.2.10
  `-d:release`, embedded mode, persistence disabled
- Reproduction: `orbelias rag-bench --n=8000 --queries=80 --budget=20
  --routed-budget=3`
