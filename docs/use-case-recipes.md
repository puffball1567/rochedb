...
This page shows practical OrbeliasDB layouts for ordinary applications. The goal
is not to replace every database pattern. It is to show where OrbeliasDB's
...
Many web screens have a broad list view and a focused detail view. In OrbeliasDB,
the list can live at a broader coordinate while the detail view reads the
specific coordinate and nearby subrings.

```sh
orbelias put --ring=users --payload='{"id":"u123","name":"Ada","status":"active"}' --codec=json
orbelias put --near=users/123 --ring=profile --payload='{"kind":"profile","name":"Ada","tier":"pro"}' --codec=json
orbelias put --near=users/123 --ring=orders --payload='{"kind":"order","orderNo":"A-001","total":120}' --codec=json
orbelias put --near=users/123 --ring=billing --payload='{"kind":"billing","plan":"annual"}' --codec=json

orbelias get --ring=users --limit=20 --selection='{ id name status }'
orbelias get --ring=users/123 --subring=profile,orders,billing --selection='{ kind name tier orderNo total plan }'
```
...
```sh
orbelias put --ring=accounts/acme --payload='{"kind":"account","name":"Acme"}' --codec=json
orbelias put --near=accounts/acme --ring=members --payload='{"kind":"member","user":"u1","role":"owner"}' --codec=json
orbelias put --near=accounts/acme --ring=members --payload='{"kind":"member","user":"u2","role":"viewer"}' --codec=json
orbelias put --near=accounts/acme --ring=audit --payload='{"kind":"audit","event":"member.invited","user":"u2"}' --codec=json

orbelias get --ring=accounts/acme --subring=members --filter='{"role":"owner"}'
orbelias get --ring=accounts/acme --subring=audit --limit=10
```
...
OrbeliasDB should not be positioned as a financial ledger. But ordinary inventory
...
```sh
orbelias put --ring=shops/1123/products/sku-9 --payload='{"kind":"stock","sku":"sku-9","available":12}' --codec=json
orbelias get --ring=shops/1123/products/sku-9 --limit=1
```
...
db.withRingLock("shops/1123/products/sku-9", proc() =
  db.transaction(proc(tx: var OrbeliasTx) =
    tx.update(stockId, %*{
      "kind": "stock",
      "sku": "sku-9",
      "available": 11
...
simple OrbeliasDB pattern is to keep the idempotency key in a dedicated ring and
...
```sh
orbelias put --ring=tenant/acme/users --payload='{"id":"u1","name":"Ada"}' --codec=json
orbelias put --ring=tenant/acme/orders/2026 --payload='{"orderNo":"A-001","total":120}' --codec=json
orbelias put --ring=tenant/globex/users --payload='{"id":"u9","name":"Grace"}' --codec=json

orbelias get --ring=tenant/acme/users --limit=50
orbelias get --ring=tenant/acme --subring=users,orders
```
...
```sh
orbelias put --ring=users/123 --payload='{"kind":"user","name":"Ada"}' --codec=json
orbelias put --ring=shops/1123 --payload='{"kind":"shop","name":"North Shop"}' --codec=json
orbelias put --ring=products/sku-9 --payload='{"kind":"product","name":"Keyboard"}' --codec=json
orbelias put --ring=orders/A-001 --payload='{"kind":"order","orderNo":"A-001"}' --codec=json
...
orbelias stellar attach --stellar=commerce/order/A-001 --ring=products/sku-9
...
```sh
orbelias put --ring=docs/security/passwords --payload='{"title":"Password reset","text":"..."}' --codec=json
orbelias put --ring=docs/security/oauth --payload='{"title":"OAuth setup","text":"..."}' --codec=json
orbelias put --ring=docs/billing/invoices --payload='{"title":"Invoice download","text":"..."}' --codec=json

orbelias get --ring=docs/security --subring=passwords,oauth --selection='{ title text }'
orbelias retrieve --ring=docs/security --budget=8
```

The point is not that OrbeliasDB magically understands documents. The application,
import rule, or pipeline gives OrbeliasDB useful coordinates, and OrbeliasDB makes
those coordinates part of the read path.

## When Not To Use These Patterns

Avoid forcing OrbeliasDB into workflows that need:
...
OrbeliasDB works best when the application already has meaningful locality:
