## orbelias/wire — クラスタ用ワイヤプロトコルとクライアント側トランスポート（設計書 §14）
##
...
proc socketFor(c: ClusterClient, node: int): Socket =
  ...
  if c.tls:
    when defined(ssl):
      ...
      raise newException(IOError, "TLS support requires building OrbeliasDB with -d:ssl")
  if c.username.len > 0:
...
proc transferReq*(c: ClusterClient, node: int, parent: uint64, seq: uint32,
                  ...
                  codec = pcRaw,
                  timeoutMs = 10_000) =
  ## Inter-node handoff. orbeliasd is single-threaded, so callers should pass a
