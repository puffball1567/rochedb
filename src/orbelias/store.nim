## orbelias/store — 粒子ストア（設計書 §16 永続化）
##
...
##
## 現行 WAL は `!ORBELIASDB-WAL 2` の magic/version 行から始まり、各論理
...
const
  ...
  WalMagicLine = "!ORBELIASDB-WAL 2"
  WalRecordTag = "@"
  EncryptedBackupMagic = "ORBELIASDB-BACKUP-SECRETBOX-V1\n"
  AppliedUniverseSyncRetention =
    when defined(orbeliasTestSmallLimits): 3
    else: 100_000
...
proc validateStellarMapBlob(stellar, raw: string, allowDeleted = false): JsonNode =
  ...
  ## OrbeliasDB owns coordinate normalization; Store only enforces replayable shape.
  if stellar.len == 0:
...
proc acquireDataDirLock(dir: string): cint =
  when defined(windows):
    ...
    let lockPath = dir / ".orbelias.lock"
...
proc backupKey(passphrase: string): SecretBoxKey =
  ...
  secretBoxKeyFromBytes(genericHash("orbeliasdb-backup-v1\0" & passphrase,
                                    SecretBoxKeyBytes))
...
when defined(orbeliasTestFailpoints):
...
proc openStore*(dir: string, durability: StoreDurability = durBuffered): Store =
  ## dir == "" ならメモリのみ。指定時は dir/orbelias.log に追記・起動時に再生。
...
proc backupEncrypted*(s: Store, dstDir, passphrase: string): StoreBackupStats =
  ## 現在の Store 状態を secretbox で暗号化した snapshot として dstDir/orbelias.backup に退避する。
  if dstDir.len == 0:
    raise newException(ValueError, "backup destination is empty")
  createDir(dstDir)
  let dst = dstDir / "orbelias.backup"
  let tmpPlain = dstDir / "orbelias.log.tmp"
  let tmpEnc = dst & ".tmp"
  if s.persistent:
    s.flushMaybe(force = true)
...
proc verifyBackup*(backupDir: string): StoreBackupStats =
  ## backupDir/orbelias.log を復元前に strict 検証する。通常 openStore の
  ## tail repair とは違い、backup 検証では壊れた snapshot を拒否する。
  if backupDir.len == 0:
    raise newException(ValueError, "backup directory is required")
  let src = backupDir / "orbelias.log"
  if not fileExists(src):
    raise newException(IOError, "backup orbelias.log not found: " & src)
  result = snapshotStatsFromFile(src, src)

proc verifyEncryptedBackup*(backupDir, passphrase: string): StoreBackupStats =
  ## backupDir/orbelias.backup を復号し、復元前に strict 検証する。
  if backupDir.len == 0:
    raise newException(ValueError, "backup directory is required")
  let src = backupDir / "orbelias.backup"
  if not fileExists(src):
  ...
  let validateDir = createTempDir("orbelias-verify", "")
  let validateTmp = validateDir / "orbelias.log"
...
proc restoreBackup*(backupDir, targetDir: string, overwrite = false,
  ...
  ## backupDir/orbelias.log を targetDir/orbelias.log として復元する。
  ...
  createDir(targetDir)
  let dst = targetDir / "orbelias.log"
...
proc restoreEncryptedBackup*(backupDir, targetDir, passphrase: string,
                             overwrite = false,
                             durability: StoreDurability = durBuffered): StoreBackupStats =
  ## backupDir/orbelias.backup を復号し、targetDir/orbelias.log として復元する。
  if backupDir.len == 0 or targetDir.len == 0:
    raise newException(ValueError, "backup and target directories are required")
  let src = backupDir / "orbelias.backup"
  if not fileExists(src):
...
proc putWarpJob*(s: Store, jobId: uint64, blob: string) =
  ## OrbeliasDB layer が解釈する warp job snapshot を保存する。
...
proc putUniverseSyncEvent*(s: Store, eventId: uint64, blob: string) =
  ## OrbeliasDB layer が解釈する universe sync event snapshot を保存する。
