## OrbeliasDB — 公開 API（設計書 §13–§16）
##
## 学習コスト最小の表面。まずこの3語だけで使い始められる:
##
## ```nim
## var db = orbeliasdb.open()          # 組み込み（単一プロセス）
## let id = db.put("hello")
## echo db.get(id)
## ```
##
## そのままの API でスケールアウトできる（サーバは orbeliasd を並べるだけ）:
##
## ```nim
...
## `orbeliasdb.open(dataDir = "/var/lib/orbelias")` / `orbeliasd --data=DIR`

import std/[algorithm, tables, hashes, json, times, strutils, os]
import orbelias/[core, store, select, wire, field, vector_backend, faiss_backend,
...
type
  OrbeliasDurability* = StoreDurability
  ...
  OrbeliasLockScope* = enum
    rlsRing
    rlsStellar

  OrbeliasLockToken* = object
  ...
  SearchProfile* = object
    ...
    ## RDB の optimizer hint を、OrbeliasDB では自然な語彙で表す。
    amount*: ResultAmount
    scope*: SearchScope
    depth*: SearchDepth
    note*: string
  ...
  OrbeliasDb* = ref object
    ...
    activeLocks: Table[string, OrbeliasLockState]
    lockFence: uint64
  ...
  GalaxyRouter* = ref object
    galaxies: Table[string, OrbeliasDb]

  OrbeliasTx* = ref object
  ...
  OrbeliasListPage* = object
    ## cursor pagination の結果。nextCursor が空なら次ページなし。
    items*: seq[OrbeliasRecord]
    nextCursor*: string

  OrbeliasReadSortDirection* = enum
    rsAsc
    rsDesc

  OrbeliasPaginationMode* = enum
    rpOff
    rpOn

  OrbeliasReadOptions* = object
    ...
    limit*: int
    cursor*: string
    pagination*: OrbeliasPaginationMode
    page*: int
    pageLimit*: int
    sortField*: string
    sortDirection*: OrbeliasReadSortDirection

  OrbeliasFilterBuilder* = object
  ...
  OrbeliasStellarRingPage* = object
    ring*: string
    count*: int
    items*: seq[OrbeliasRecord]

  OrbeliasStellarPage* = object
    root*: string
    maxDepth*: int
    branchBudget*: int
    ringsVisited*: int
    count*: int
    rings*: seq[OrbeliasStellarRingPage]
...
const
  ...
  DefaultPeriod = 60.0
  RetrievalEnvelopeSchema* = "orbeliasdb.retrieval.v1"
  RetrievalEnvelopeVersion* = 1
  AtlasSchema* = "orbeliasdb.atlas.v1"
  AtlasVersion* = 1

proc parentRingName(name: string): string
proc addRingChild(db: OrbeliasDb, parent, child: uint64)
proc ringNameOf(db: OrbeliasDb, key: uint64): string
proc ringKey(db: OrbeliasDb, name: string, persist = true): uint64
proc ringKeyForRead(db: OrbeliasDb, ring: string): uint64
proc descendantRingKeys(db: OrbeliasDb, root: uint64, maxDepth, branchBudget: int): seq[uint64]
proc addUniqueRingKey(keys: var seq[uint64], key: uint64)
proc stellarNeighborKeys(db: OrbeliasDb, root: uint64, maxDepth, branchBudget: int): seq[uint64]
proc put*(db: OrbeliasDb, encoded: EncodedPayload, ring: string = "default",
          vec: seq[float32] = @[]): OrbeliasId
proc put*(db: OrbeliasDb, payload: string, ring: string = "default",
          vec: seq[float32] = @[]): OrbeliasId
proc enqueueUniverseSyncEvent*(db: OrbeliasDb, sourceUniverse, sourceGalaxy,
                               ring, payload: string,
                               vec: seq[float32] = @[],
                               codec = pcRaw,
                               op = "put", logicalKey = "",
                               timestamp = -1.0,
                               eventKey = ""): uint64
proc stageUniverseSyncEvent(db: OrbeliasDb, tx: StoreTxn, sourceUniverse,
...
proc rebuildStellarMembership(db: OrbeliasDb) =
...
proc loadStellarMap(db: OrbeliasDb, raw: string) =
  if raw.len == 0:
    return
...
proc lockKey(scope: OrbeliasLockScope, coordinate: string): string =
  case scope
  of rlsRing: "ring:" & normalizedCoordinate(coordinate)
  of rlsStellar: "stellar:" & normalizedCoordinate(coordinate)

proc purgeExpiredLocks(db: OrbeliasDb, now = epochTime()) =
...
proc newLockToken(db: OrbeliasDb, keys: seq[string]): tuple[token: string, fence: uint64] =
...
proc acquireLockKeys(db: OrbeliasDb, scope: OrbeliasLockScope, coordinate: string,
                     keys: seq[string], ttlSeconds = 30.0,
                     waitMs = 0): OrbeliasLockToken =
  ...
  while true:
    ...
    if busy.len == 0:
      ...
      for key in uniqueKeys:
        db.activeLocks[key] = OrbeliasLockState(scope: scope,
                                             coordinate: coordinate,
                                             token: generated.token,
                                             expiresAt: expiresAt)
      return OrbeliasLockToken(scope: scope, coordinate: coordinate,
...
proc acquireRingLock*(db: OrbeliasDb, ring: string, ttlSeconds = 30.0,
                      waitMs = 0): OrbeliasLockToken =
...
proc acquireStellarLock*(db: OrbeliasDb, stellar: string, ttlSeconds = 30.0,
                         waitMs = 0): OrbeliasLockToken =
...
proc releaseLock*(db: OrbeliasDb, token: OrbeliasLockToken) =
...
proc lockActive*(db: OrbeliasDb, token: OrbeliasLockToken): bool =
...
proc withRingLock*(db: OrbeliasDb, ring: string, body: proc(),
...
proc withStellarLock*(db: OrbeliasDb, stellar: string, body: proc(),
                      ttlSeconds = 30.0, waitMs = 0) =
...
proc open*(nodes: int = 8, dataDir: string = "",
           durability: OrbeliasDurability = durBuffered): OrbeliasDb =
  ...
  result = OrbeliasDb(mode: mEmbedded,
                   tbl: tbl,
...
proc connect*(peers: string, username: string = "", password: string = "",
              ...
              tlsInsecureSkipVerify: bool = false): OrbeliasDb =
  ## クラスタモードで開く。peers = "host:port,host:port,..."（orbeliasd の並び順）。
  ## 時計は wall clock。API は open() と同じに使える。
  let ps = parsePeers(peers)
  OrbeliasDb(mode: mCluster,
...
proc galaxy*(r: GalaxyRouter, name: string): OrbeliasDb =
  r.galaxies[name]
...
proc setGalaxyDescription*(db: OrbeliasDb, description: string) =
...
proc getGalaxyDescription*(db: OrbeliasDb): string =
  db.galaxyDescription

proc setRingDescription*(db: OrbeliasDb, ring, description: string) =
...
proc getRingDescription*(db: OrbeliasDb, ring: string): string =
...
proc configureRingPayloadProfile*(db: OrbeliasDb, ring: string,
...
proc ringPayloadProfile*(db: OrbeliasDb, ring: string): RingPayloadProfile =
  let key = db.ringKeyForRead(ring)
  if key == 0'u64:
...
proc configureTimeOrbitProfile*(db: OrbeliasDb, ring: string,
...
proc timeOrbitProfile*(db: OrbeliasDb, ring: string): TimeOrbitProfile =
  let key = db.ringKeyForRead(ring)
  if key == 0'u64:
...
proc compact*(db: OrbeliasDb): CompactStats =
  ## 組み込み永続 Store の WAL を生存レコードだけに再構築する。
  ## cluster 接続では各 orbeliasd 側の管理操作として実行する。
  if db.mode != mEmbedded:
    raise newException(ValueError, "cluster connection cannot compact remote stores")
  db.st.compact()

proc localityReport*(db: OrbeliasDb): LocalityReport =
...
proc backup*(db: OrbeliasDb, dstDir: string): BackupStats =
...
proc backupEncrypted*(db: OrbeliasDb, dstDir, passphrase: string): BackupStats =
...
proc restoreBackup*(backupDir, dataDir: string, overwrite = false,
                    durability: OrbeliasDurability = durBuffered): BackupStats =
...
proc dump*(db: OrbeliasDb, path: string = "", includeVectors = true): DumpStats =
  ...
  try:
    ...
    for _, p in db.st.items:
      var doc = %*{
        "type": "document",
        "id": $(OrbeliasId(parent: p.parent, epoch: db.tbl.epoch,
...
proc importJsonl*(db: OrbeliasDb, path: string, defaultRing = "imported",
  ...
  for line in lines(path):
    ...
    try:
      ...
      if nodeType == "meta" and
          node{"format"}.getStr("") == "orbeliasdb.dump.v1":
        orbeliasDumpMode = true
        inc result.skipped
        continue
...
proc configureVectorBackend*(db: OrbeliasDb, kind: VectorBackendKind) =
...
proc configurePlannerBackend*(db: OrbeliasDb, kind: PlannerBackendKind) =
  ## retrieval planner backend を選ぶ。
  ## OrbeliasDB core は deterministic heuristic planner を使う。
  case kind
  of pbHeuristic:
...
proc configureRetrievalTuning*(db: OrbeliasDb, profile: string,
...
proc configureSearchProfile*(db: OrbeliasDb, name: string,
...
proc retrievalTuning*(db: OrbeliasDb, profile = "default"): RetrievalTuning =
  if profile in db.retrievalTunings:
    db.retrievalTunings[profile]
  else:
    defaultRetrievalTuning()

proc nowT(db: OrbeliasDb): float {.inline.} =
  case db.mode
  of mEmbedded: db.clock
  of mCluster: epochTime()

proc now*(db: OrbeliasDb): float =
  ## DB 時計の現在時刻 [s]。組み込み=手動クロック、クラスタ=実時間。
  db.nowT

proc advance*(db: OrbeliasDb, dt: float) =
...
proc orbitOf(db: OrbeliasDb, id: OrbeliasId): Orbit =
...
proc configureRing*(db: OrbeliasDb, ring: string, period: float) =
...
proc configureWriteAckMode*(db: OrbeliasDb, mode: WriteAckMode) =
...
proc configureRingWriteAckMode*(db: OrbeliasDb, ring: string,
                                mode: WriteAckMode) =
...
proc configureRingApplyPolicy*(db: OrbeliasDb, ring: string,
...
proc ringApplyPolicy*(db: OrbeliasDb, ring: string): RingApplyPolicy =
  let key = db.ringKeyForRead(ring)
  db.ringApplyPolicies.getOrDefault(key, defaultRingApplyPolicy())

proc writeAckModeForRing(db: OrbeliasDb, ringKey: uint64): WriteAckMode =
  db.ringWriteAckModes.getOrDefault(ringKey, db.defaultWriteAckMode)

proc writeAckModeForOps(db: OrbeliasDb, ops: seq[TxWireOp]): WriteAckMode =
  if ops.len == 0:
...
proc markPendingLandingReads(db: OrbeliasDb, ops: seq[TxWireOp]) =
  if db.mode != mCluster:
    return
  for op in ops:
    db.pendingLandingReads[(op.parent, op.seq)] = true

proc clearPendingLandingRead(db: OrbeliasDb, id: OrbeliasId) =
  if db.mode == mCluster:
    db.pendingLandingReads.del((id.parent, id.seq))

proc hasPendingLandingRead(db: OrbeliasDb, id: OrbeliasId): bool =
  db.mode == mCluster and db.pendingLandingReads.getOrDefault((id.parent, id.seq), false)

proc waitClusterTxApplied*(db: OrbeliasDb, txid: uint64, timeoutMs = 10_000,
                           pollMs = 20): bool =
...
proc put*(db: OrbeliasDb, encoded: EncodedPayload, ring: string = "default",
  ...
  of mEmbedded:
    ...
    result = OrbeliasId(parent: key, epoch: db.tbl.epoch, seq: seq, tWrite: db.clock)
  ...
  of mCluster:
    ...
    result = OrbeliasId(parent: key, epoch: db.tbl.epoch, seq: seq, tWrite: tWrite)

proc put*(db: OrbeliasDb, payload: string, ring: string = "default",
          vec: seq[float32] = @[]): OrbeliasId =
  db.put(encodedPayload(payload), ring, vec)

proc putUsingRingProfile*(db: OrbeliasDb, payload: string,
                          ring: string = "default",
                          vec: seq[float32] = @[]): OrbeliasId =
...
proc putTime*(db: OrbeliasDb, payload: string, ring: string,
              timestampMs: int64, vec: seq[float32] = @[]): OrbeliasId =
...
proc putTime*(db: OrbeliasDb, doc: JsonNode, ring: string,
              timestampMs: int64, vec: seq[float32] = @[]): OrbeliasId =
  db.putTime($doc, ring, timestampMs, vec)

proc put*(db: OrbeliasDb, doc: JsonNode, ring: string = "default",
          vec: seq[float32] = @[]): OrbeliasId =
  ## 構造化ドキュメントの書き込み。query（選択取得）の対象になる。
  db.put(encodedPayload($doc, pcJson), ring, vec)

proc putSynced*(db: OrbeliasDb, encoded: EncodedPayload,
                sourceUniverse, sourceGalaxy: string,
                ring: string = "default", vec: seq[float32] = @[],
                logicalKey = ""): OrbeliasId =
  ...
  try:
    ...
    let seq = db.st.nextSeq(key)
    result = OrbeliasId(parent: key, epoch: db.tbl.epoch, seq: seq,
                     tWrite: db.clock)
...
proc putSynced*(db: OrbeliasDb, payload: string, sourceUniverse, sourceGalaxy: string,
                ring: string = "default", vec: seq[float32] = @[],
                logicalKey = ""): OrbeliasId =
  db.putSynced(encodedPayload(payload), sourceUniverse, sourceGalaxy,
               ring, vec, logicalKey)

proc putSynced*(db: OrbeliasDb, doc: JsonNode, sourceUniverse, sourceGalaxy: string,
                ring: string = "default", vec: seq[float32] = @[],
                logicalKey = ""): OrbeliasId =
...
proc beginTransaction*(db: OrbeliasDb): OrbeliasTx =
  ## transaction を開始する。
  ## embedded: 単一 Store atomic transaction。
  ## cluster: node0 landing zone に commit intent を置き、各 owner へ非同期 apply。
  case db.mode
  of mEmbedded:
    OrbeliasTx(db: db, tx: db.st.beginTxn())
  of mCluster:
    OrbeliasTx(db: db, clusterTxId: db.client.txBeginReq(0))

proc put*(tx: OrbeliasTx, encoded: EncodedPayload, ring: string = "default",
          vec: seq[float32] = @[]): OrbeliasId =
  ...
  of mEmbedded:
    ...
    result = OrbeliasId(parent: key, epoch: tx.db.tbl.epoch, seq: seq, tWrite: tx.db.clock)
  ...
  of mCluster:
    ...
    result = OrbeliasId(parent: key, epoch: tx.db.tbl.epoch, seq: seq, tWrite: tWrite)
    tx.clusterOps.add TxWireOp(parent: key, seq: seq, period: ri.period,
                               head: ri.headAngle, tWrite: tWrite,
                               payload: encoded.data, codec: encoded.codec, vec: normVec)

proc put*(tx: OrbeliasTx, payload: string, ring: string = "default",
          vec: seq[float32] = @[]): OrbeliasId =
  tx.put(encodedPayload(payload), ring, vec)

proc put*(tx: OrbeliasTx, doc: JsonNode, ring: string = "default",
          vec: seq[float32] = @[]): OrbeliasId =
  tx.put(encodedPayload($doc, pcJson), ring, vec)

proc remove*(tx: OrbeliasTx, id: OrbeliasId) =
...
proc update*(tx: OrbeliasTx, id: OrbeliasId, encoded: EncodedPayload,
             vec: seq[float32] = @[]) =
...
proc update*(tx: OrbeliasTx, id: OrbeliasId, payload: string,
             vec: seq[float32] = @[]) =
  tx.update(id, encodedPayload(payload), vec)

proc update*(tx: OrbeliasTx, id: OrbeliasId, doc: JsonNode,
             vec: seq[float32] = @[]) =
  tx.update(id, encodedPayload($doc, pcJson), vec)

proc commit*(tx: OrbeliasTx) =
  doAssert not tx.closed, "transaction is closed"
  case tx.db.mode
  of mEmbedded:
    tx.tx.commit()
...
proc commit*(tx: OrbeliasTx, ackMode: WriteAckMode) =
...
proc rollback*(tx: OrbeliasTx) =
  if not tx.closed:
    if tx.db.mode == mEmbedded:
      tx.tx.rollback()
    tx.closed = true

proc transaction*(db: OrbeliasDb, body: proc(tx: OrbeliasTx)) =
...
proc batchPutAtomic*(db: OrbeliasDb, payloads: seq[string],
                     ring: string = "default",
                     vecs: seq[seq[float32]] = @[]): seq[OrbeliasId] =
...
proc batchPutAtomic*(db: OrbeliasDb, docs: seq[JsonNode],
                     ring: string = "default",
                     vecs: seq[seq[float32]] = @[]): seq[OrbeliasId] =
...
proc batchUpdateAtomic*(db: OrbeliasDb, ids: seq[OrbeliasId],
                        payloads: seq[string],
...
proc batchDeleteAtomic*(db: OrbeliasDb, ids: seq[OrbeliasId]) =
...
proc fetchClusterPayload(db: OrbeliasDb, id: OrbeliasId, selection: string,
  ...
  for node in [primary, (primary + n - 1) mod n, primary]:
    ...
    if r.forwarded:
      ...
      return db.fetchClusterPayload(OrbeliasId(parent: r.newParent, epoch: id.epoch,
...
proc get*(db: OrbeliasDb, id: OrbeliasId): string =
...
proc getEncoded*(db: OrbeliasDb, id: OrbeliasId): EncodedPayload =
...
proc batchGet*(db: OrbeliasDb, ids: seq[OrbeliasId]): seq[string] =
  ...
  of mCluster:
    var byNode = initTable[int, seq[tuple[idx: int, id: OrbeliasId]]]()
    let t = epochTime()
...
proc exists*(db: OrbeliasDb, id: OrbeliasId): bool =
...
proc remove*(db: OrbeliasDb, id: OrbeliasId) =
...
proc deleteById*(db: OrbeliasDb, id: OrbeliasId) =
  ## ORM で直感的に使うための alias。
  db.remove(id)

proc update*(db: OrbeliasDb, id: OrbeliasId, encoded: EncodedPayload,
             vec: seq[float32] = @[]) =
...
proc update*(db: OrbeliasDb, id: OrbeliasId, payload: string,
             vec: seq[float32] = @[]) =
  db.update(id, encodedPayload(payload), vec)

proc update*(db: OrbeliasDb, id: OrbeliasId, doc: JsonNode,
             vec: seq[float32] = @[]) =
...
proc patch*(db: OrbeliasDb, id: OrbeliasId, patchDoc: JsonNode): JsonNode =
...
proc countByRing*(db: OrbeliasDb, ring: string): int =
...
proc listByRing*(db: OrbeliasDb, ring: string, limit = 100,
                 cursor = ""): OrbeliasListPage =
  ...
  if db.mode == mCluster:
    ...
    for row in rows:
      ...
      if result.items.len >= limit:
        ...
        break
      result.items.add OrbeliasRecord(
        id: OrbeliasId(parent: row.parent, epoch: db.tbl.epoch, seq: row.seq,
                    tWrite: row.tWrite),
  ...
  for itemKey in db.st.itemsByRing.getOrDefault(key, @[]):
    ...
    result.items.add OrbeliasRecord(
      id: OrbeliasId(parent: p.parent, epoch: db.tbl.epoch, seq: p.seq,
                  tWrite: p.tWrite),
      payload: p.payload, codec: p.codec)
    lastSeq = itemKey[1].int64
    inc emitted

proc defaultReadOptions*(): OrbeliasReadOptions =
  OrbeliasReadOptions(
    filter: newJObject(),
    selection: "",
    limit: 100,
...
proc defaultStellarOptions*(): OrbeliasStellarOptions =
  OrbeliasStellarOptions(
    filter: newJObject(),
    selection: "",
    limitPerRing: 20,
    maxDepth: 1,
...
proc orbeliasFilter*(): OrbeliasFilterBuilder =
  ## Start a safe read-filter builder.
  OrbeliasFilterBuilder(node: newJObject())

proc toJson*(builder: OrbeliasFilterBuilder): JsonNode =
  ## Return a defensive JSON object copy suitable for OrbeliasReadOptions.filter.
  if builder.node.isNil:
    return newJObject()
  if builder.node.kind != JObject:
    raise newException(ValueError, "Orbelias filter builder must contain a JSON object")
  builder.node.copy()

proc withFilterValue(builder: OrbeliasFilterBuilder; key: string;
                     value: JsonNode): OrbeliasFilterBuilder =
  if key.len == 0:
...
proc eq*(builder: OrbeliasFilterBuilder; key: string;
         value: JsonNode): OrbeliasFilterBuilder =
  ## Add an equality match for a JSON value.
  builder.withFilterValue(key, value)

proc eq*(builder: OrbeliasFilterBuilder; key, value: string): OrbeliasFilterBuilder =
  ## Add an equality match for a string value.
  builder.withFilterValue(key, %value)

proc eq*(builder: OrbeliasFilterBuilder; key: string;
         value: SomeInteger): OrbeliasFilterBuilder =
  ## Add an equality match for an integer value.
  builder.withFilterValue(key, %value)

proc eq*(builder: OrbeliasFilterBuilder; key: string;
         value: SomeFloat): OrbeliasFilterBuilder =
  ## Add an equality match for a floating-point value.
  builder.withFilterValue(key, %value)

proc eq*(builder: OrbeliasFilterBuilder; key: string;
         value: bool): OrbeliasFilterBuilder =
  ## Add an equality match for a boolean value.
  builder.withFilterValue(key, %value)

proc id*(builder: OrbeliasFilterBuilder; rawId: string): OrbeliasFilterBuilder =
  ## Match a OrbeliasDB raw/string ID using the same `id` filter field accepted by
  ## readRing and CLI `--filter='{"id":"..."}'`.
  builder.eq("id", rawId)

proc withFilter*(options: OrbeliasReadOptions;
                 builder: OrbeliasFilterBuilder): OrbeliasReadOptions =
  ## Return read options with a builder-produced filter.
  result = options
  result.filter = builder.toJson()

proc withFilter*(options: OrbeliasStellarOptions;
                 builder: OrbeliasFilterBuilder): OrbeliasStellarOptions =
  ## Return stellar read options with a builder-produced filter.
  result = options
  result.filter = builder.toJson()

proc normalizedReadOptions(options: OrbeliasReadOptions): OrbeliasReadOptions =
  result = options
...
proc recordMatchesReadFilter(item: OrbeliasRecord, filterNode: JsonNode): bool =
...
proc projectReadRecord(item: OrbeliasRecord, selection: string): OrbeliasRecord =
  result = item
  if selection.len == 0:
    return
  if not item.codec.supportsJsonProjection:
...
proc compareReadRecords(a, b: OrbeliasRecord, options: OrbeliasReadOptions): int =
...
proc readRing*(db: OrbeliasDb, ring: string,
               options: OrbeliasReadOptions = defaultReadOptions()): OrbeliasReadPage =
  ...
  var matched: seq[OrbeliasRecord] = @[]
  ...
  matched.sort(proc(a, b: OrbeliasRecord): int = compareReadRecords(a, b, opts))
...
proc eventTimeMsOf(item: OrbeliasRecord): tuple[ok: bool, value: int64] =
  if not item.codec.supportsJsonProjection:
    return (false, 0'i64)
  try:
...
proc compareTimeRecords(a, b: OrbeliasRecord): int =
...
proc readTime*(db: OrbeliasDb, ring: string, fromMs, toMs: int64,
               options: OrbeliasReadOptions = defaultReadOptions(),
               maxBuckets = 1024): OrbeliasTimeReadPage =
...
proc anchorRingName(db: OrbeliasDb, anchor: OrbeliasId): string =
  result = db.ringKeyNames.getOrDefault(anchor.parent, "")
  if result.len == 0:
    raise newException(ValueError,
      "anchor ring is unknown in this OrbeliasDB handle; configure or read the ring map first")

proc nearRing*(db: OrbeliasDb, anchor: OrbeliasId, relation: string): string =
...
proc putNear*(db: OrbeliasDb, baseRing: string, encoded: EncodedPayload,
              ring: string, vec: seq[float32] = @[]): OrbeliasId =
...
proc putNear*(db: OrbeliasDb, baseRing, payload, ring: string,
              vec: seq[float32] = @[]): OrbeliasId =
  db.putNear(baseRing, encodedPayload(payload), ring = ring, vec = vec)

proc putNear*(db: OrbeliasDb, baseRing: string, doc: JsonNode, ring: string,
              vec: seq[float32] = @[]): OrbeliasId =
  db.putNear(baseRing, encodedPayload($doc, pcJson), ring = ring, vec = vec)

proc putNear*(db: OrbeliasDb, anchor: OrbeliasId, encoded: EncodedPayload,
              relation = "context", vec: seq[float32] = @[]): OrbeliasId =
...
proc putNear*(db: OrbeliasDb, anchor: OrbeliasId, payload: string,
              relation = "context", vec: seq[float32] = @[]): OrbeliasId =
  db.putNear(anchor, encodedPayload(payload), relation = relation, vec = vec)

proc putNear*(db: OrbeliasDb, anchor: OrbeliasId, doc: JsonNode,
              relation = "context", vec: seq[float32] = @[]): OrbeliasId =
  db.putNear(anchor, encodedPayload($doc, pcJson), relation = relation, vec = vec)

proc attachStellar*(db: OrbeliasDb, stellar, ring: string) =
...
proc detachStellar*(db: OrbeliasDb, stellar, ring: string) =
...
proc stellarMembers*(db: OrbeliasDb, stellar: string): seq[string] =
  db.stellarMembers.getOrDefault(normalizedCoordinate(stellar), @[])

proc stellarCoordinatesFor*(db: OrbeliasDb, ring: string): seq[string] =
  db.stellarByMember.getOrDefault(normalizedCoordinate(ring), @[])

proc normalizedStellarOptions(options: OrbeliasStellarOptions): OrbeliasStellarOptions =
  result = options
  if result.filter.isNil:
    result.filter = newJObject()
...
proc readStellar*(db: OrbeliasDb, root: string,
                  options: OrbeliasStellarOptions = defaultStellarOptions()): OrbeliasStellarPage =
  ...
  for key in keys:
    ...
    let page = db.readRing(ringName, OrbeliasReadOptions(
      filter: opts.filter,
    ...
    if page.items.len == 0:
      continue
    result.rings.add OrbeliasStellarRingPage(ring: ringName,
                                          count: page.count,
                                          items: page.items)
    result.count += page.count
  result.ringsVisited = keys.len

proc batchPut*(db: OrbeliasDb, payloads: seq[string], ring: string = "default",
               vecs: seq[seq[float32]] = @[]): seq[OrbeliasId] =
...
proc batchPut*(db: OrbeliasDb, docs: seq[JsonNode], ring: string = "default",
               vecs: seq[seq[float32]] = @[]): seq[OrbeliasId] =
  for i, doc in docs:
    let vec = if i < vecs.len: vecs[i] else: @[]
    result.add db.put(doc, ring = ring, vec = vec)

proc findWarpJob(db: OrbeliasDb, jobId: uint64): int =
...
proc persistWarpJob(db: OrbeliasDb, job: WarpJob) =
...
proc persistUniverseSyncEvent(db: OrbeliasDb, event: UniverseSyncEvent) =
  if db.mode == mEmbedded and not db.st.isNil:
...
proc deleteUniverseSyncEventAt(db: OrbeliasDb, idx: int) =
  let eventId = db.universeSyncEvents[idx].id
  db.universeSyncEvents.delete(idx)
  if db.mode == mEmbedded and not db.st.isNil:
    db.st.deleteUniverseSyncEvent(eventId)

proc pendingUniverseSyncCoalesceIndexes(db: OrbeliasDb, sourceUniverse,
...
proc coalescePendingUniverseSyncEvents(db: OrbeliasDb, sourceUniverse,
                                       sourceGalaxy, ring, op,
                                       logicalKey: string,
                                       policy: RingApplyPolicy) =
...
proc universeSyncEventReady(db: OrbeliasDb, event: UniverseSyncEvent): bool =
...
proc markUniverseSyncDelayed*(db: OrbeliasDb, eventId: uint64, retryAt: float) =
  for i, event in db.universeSyncEvents:
    if event.id == eventId:
      var updated = event
...
proc markUniverseSyncFailure*(db: OrbeliasDb, eventId: uint64,
...
proc universeSyncEvents*(db: OrbeliasDb,
...
proc applyUniverseSyncEvent*(db: OrbeliasDb, event: UniverseSyncEvent): bool =
...
proc ackUniverseSyncEvent*(db: OrbeliasDb, eventId: uint64): UniverseSyncEvent =
...
proc pruneAckedUniverseSyncEvents*(db: OrbeliasDb): int =
...
proc syncUniverseOnce*(source, target: OrbeliasDb,
...
proc enqueueWarp*(db: OrbeliasDb, rings: seq[string], whereField: string,
...
proc warpStatus*(db: OrbeliasDb, jobId: uint64): WarpJob =
  let idx = db.findWarpJob(jobId)
  if idx < 0:
    raise newException(KeyError, "warp job not found")
  db.warpJobs[idx]

proc ackWarp*(db: OrbeliasDb, jobId: uint64): WarpJob =
...
proc pruneAckedWarpJobs*(db: OrbeliasDb): int =
...
proc warpStep*(db: OrbeliasDb, jobId: uint64, maxRecords = 100,
               now = -1.0): WarpJob =
  ...
  var updates: seq[tuple[id: OrbeliasId, doc: JsonNode]] = @[]
  try:
...
proc warpDrain*(db: OrbeliasDb, jobId: uint64, maxSteps = 1000,
...
proc batchRemove*(db: OrbeliasDb, ids: seq[OrbeliasId]) =
  ## 複数 ID を削除する。v1 は embedded 専用。
  doAssert db.mode == mEmbedded, "batchRemove は embedded mode 専用"
  for id in ids:
    db.remove(id)

proc batchDelete*(db: OrbeliasDb, ids: seq[OrbeliasId]) =
  db.batchRemove(ids)

proc query*(db: OrbeliasDb, id: OrbeliasId, prepared: PreparedSelection): JsonNode =
...
proc query*(db: OrbeliasDb, id: OrbeliasId, selection: string): JsonNode =
  ## Convenience form. Reuse PreparedSelection on hot paths.
  db.query(id, prepareSelection(selection))

proc contains*(db: OrbeliasDb, id: OrbeliasId): bool =
...
proc ringMetrics*(db: OrbeliasDb): seq[RingMetric]
proc ringSummaries*(db: OrbeliasDb, queryVec: seq[float32] = @[]): seq[OrbeliasRingSummary]

proc `$`*(id: OrbeliasId): string =
  $id.parent & ":" & $id.seq

proc id*(builder: OrbeliasFilterBuilder; orbeliasId: OrbeliasId): OrbeliasFilterBuilder =
  ## Match a OrbeliasDB ID without asking callers to manually stringify it.
  builder.id($orbeliasId.parent & ":" & $orbeliasId.epoch & ":" &
             $orbeliasId.seq & ":" & $orbeliasId.tWrite)
...
proc parentRingKey(db: OrbeliasDb, key: uint64): uint64 =
...
proc siblingRingKeys(db: OrbeliasDb, root: uint64, siblingBudget: int): seq[uint64] =
...
proc ringSummaryTable(db: OrbeliasDb, queryVec: seq[float32],
                      ringKeys: seq[uint64]): Table[uint64, OrbeliasRingSummary] =
  ...
  of mEmbedded:
    for ring in ringKeys:
      ...
      result[ring] = OrbeliasRingSummary(ringKey: ring, count: n,
...
proc expandPlan(db: OrbeliasDb, plan: RetrievalPlan,
  ...
  let summaryByRing =
    if computeFeatures:
      ...
      initTable[uint64, OrbeliasRingSummary]()
  var candidates: seq[RingPlanCandidate] = @[]
  for key in keys:
    let summary = summaryByRing.getOrDefault(
      key,
      OrbeliasRingSummary(ringKey: key, count: 0, centroid: @[], score: 0.0))
...
proc tunedRetrievalPlan*(db: OrbeliasDb, ring: string = "", profile = "default",
  ...
  of mEmbedded:
    ...
    for ring, e in merged:
      ...
      if distN > 0:
        meanDist /= float(distN)
      let coherence = if distN > 0: max(0.0, min(1.0, 1.0 - meanDist)) else: 0.0
      result.add OrbeliasRingSummary(ringKey: ring, count: e.n,
  ...
  of mCluster:
    ...
    for ring, e in merged:
      ...
      result.add OrbeliasRingSummary(ringKey: ring, count: e.n,
                                  ...
                                  coherence: 0.0,
                                  massG: 0.0)
  result.sort(proc(a, b: OrbeliasRingSummary): int =
...
proc atlas*(db: OrbeliasDb, queryVec: seq[float32] = @[],
  ...
  var summaryByRing = initTable[uint64, OrbeliasRingSummary]()
  ...
  for key in ringKeys:
    ...
    let ri = db.rings.getOrDefault(key, RingInfo(period: DefaultPeriod,
                                                headAngle: 0.0))
    let rs = summaryByRing.getOrDefault(key, OrbeliasRingSummary(ringKey: key))
    let docs =
...
proc fillReturnedPayloadStats(stats: var RetrieveStats; hits: seq[OrbeliasHit]) =
...
proc retrieveWithStats*(db: OrbeliasDb, queryVec: seq[float32], ring: string = "",
                        budget = 8, topRings = 0, focus = 0,
                        includeChildren = false, maxDepth = 0,
                        branchBudget = 0, profile = "",
                        amount = raNormal, scope = ssTight,
                        depth = sdNormal):
                        tuple[hits: seq[OrbeliasHit], stats: RetrieveStats,
                              plan: RetrievalPlan] =
  ...
  of mEmbedded:
    ...
    for h in rr.hits:
      result.hits.add OrbeliasHit(id: OrbeliasId(parent: h.parent, epoch: db.tbl.epoch,
        ...
        for node in 0 ..< db.client.peers.len:
          ...
          for h in rr.hits:
            result.hits.add OrbeliasHit(id: OrbeliasId(parent: h.parent, epoch: db.tbl.epoch,
                                                 seq: h.seq, tWrite: h.tWrite),
                                     score: h.score, payload: h.payload, codec: h.codec)
  result.hits.sort(proc(a, b: OrbeliasHit): int = cmp(b.score, a.score))
...
proc retrieve*(db: OrbeliasDb, queryVec: seq[float32], ring: string = "",
               budget = 8, topRings = 0, focus = 0): seq[OrbeliasHit] =
...
proc retrieveStats*(db: OrbeliasDb, queryVec: seq[float32], ring: string = "",
                    budget = 8, topRings = 0, focus = 0): RetrieveStats =
  ## retrieve の探索幅を測るための軽量 stats。
  db.retrieveWithStats(queryVec, ring = ring, budget = budget,
                       topRings = topRings, focus = focus).stats

proc retrieveTunedWithStats*(db: OrbeliasDb, queryVec: seq[float32],
...
proc retrieveTuned*(db: OrbeliasDb, queryVec: seq[float32],
                    ring: string = "", profile = "default"): seq[OrbeliasHit] =
  db.retrieveTunedWithStats(queryVec, ring = ring, profile = profile).hits

proc defaultRetrievalEnvelopeOptions*(db: OrbeliasDb, ring = ""): RetrievalEnvelopeOptions =
  ## Shelfer などの外部 runtime が使う安定 envelope の既定値。
  result.provider = "orbeliasdb"
...
proc retrievalEnvelope*(
    hits: seq[OrbeliasHit];
    stats: RetrieveStats;
    opts: RetrievalEnvelopeOptions;
    budget = 8;
    ringScoped = false): JsonNode =
  ## retrieve 結果を OrbeliasDB 依存の薄い JSON 契約に変換する。
  ## OrbeliasDB core は Shelfer に依存せず、Shelfer adapter はこの envelope を消費する。
  ...
  for h in hits:
    ...
    chunks.add %*{
      ...
      "ring": opts.ring,
      "sourceUri": "orbeliasdb://" & opts.galaxy & "/" & opts.ring & "/" & $h.id
    }
...
proc retrievalEnvelope*(db: OrbeliasDb, queryVec: seq[float32], ring: string = "",
...
proc retrievalEnvelopeTuned*(db: OrbeliasDb, queryVec: seq[float32],
...
proc retrievalEnvelopeValidationErrors*(env: JsonNode): seq[string] =
  ## OrbeliasDB retrieval envelope v1 の互換性チェック。
...
proc locate*(db: OrbeliasDb, id: OrbeliasId, at: float = -1.0): int =
...
proc nextVisit*(db: OrbeliasDb, id: OrbeliasId, node: int): float =
  ## その ID が指定ノードに次に到着する時刻（プリフェッチ・バッチの予定表）。
  db.orbitOf(id).nextArrival(db.tbl.arcStart(NodeId(node)), db.nowT)

proc nextJoin*(db: OrbeliasDb, a, b: OrbeliasId): float =
...
proc stats*(db: OrbeliasDb): seq[tuple[node, count: int]] =
...
proc health*(db: OrbeliasDb): seq[string] =
...
proc metrics*(db: OrbeliasDb): seq[string] =
...
proc shutdownCluster*(db: OrbeliasDb): seq[string] =
...
proc toRaw*(id: OrbeliasId): tuple[parent: uint64, epoch: uint32, seq: uint32, tWrite: float] =
  ## 内部用（C ABI 変換）。アプリケーションコードでは使わないこと。
  (id.parent, id.epoch, id.seq, id.tWrite)

proc fromRaw*(parent: uint64, epoch: uint32, seq: uint32, tWrite: float): OrbeliasId =
  ## 内部用（C ABI 変換）。アプリケーションコードでは使わないこと。
  OrbeliasId(parent: parent, epoch: epoch, seq: seq, tWrite: tWrite)
