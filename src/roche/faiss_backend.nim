## roche/faiss_backend — optional FAISS vector backend.
##
## This module talks to a small C ABI bridge (`libroche_faiss.so`). The main
## RocheDB build does not link FAISS directly; selecting vbFaiss requires the
## bridge library to be available at runtime.

import std/[algorithm, dynlib, os, tables]
import ./[store, vector_backend]

const FaissDynlib = "libroche_faiss.so"
...
proc candidateLibs(): seq[string] =
  let envPath = getEnv("ROCHE_FAISS_BRIDGE")
  if envPath.len > 0:
    result.add envPath
...
proc loadFaissApi(): FaissApi =
  ...
  if lib == nil:
    raise newException(ValueError,
      ...
      "or set ROCHE_FAISS_BRIDGE=/path/to/libroche_faiss.so")
  result.lib = lib
  result.newIndex = loadSym[FaissNewProc](lib, "roche_faiss_new")
  result.freeIndex = loadSym[FaissFreeProc](lib, "roche_faiss_free")
  result.add = loadSym[FaissAddProc](lib, "roche_faiss_add")
  result.search = loadSym[FaissSearchProc](lib, "roche_faiss_search")
  cachedApi = result
