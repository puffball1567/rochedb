## orbeliasdb_capi — C ABI 層（設計書 §13）
##
## ビルド:  scripts/build_capi.sh
## ヘッダ:  include/orbeliasdb.h（手書き・本ファイルと1:1対応）
##
## 規約:
##   - ハンドルは不透明ポインタ。ARC 管理の ref を GC_ref/GC_unref で寿命固定。
##   - orbelias_id は 24 バイトの値渡し struct（ヘッダと ABI 一致必須）。
##   - 例外は境界を越えない: すべて捕捉し、エラーはリターンコード / nil で返す。
##   - orbelias_get が返すバッファは呼び出し側が orbelias_free で解放する。

import std/[base64, json, tables]
import orbeliasdb

type
  OrbeliasCHandle = ref object
    db: OrbeliasDb
    closed: bool

  OrbeliasCId {.exportc: "orbelias_id", bycopy.} = object
    parent: uint64
    epoch: uint32
    seq: uint32
    t_write: cdouble

  OrbeliasCHit {.exportc: "orbelias_hit", bycopy.} = object
    id: OrbeliasCId
    score: cdouble
    payload: pointer
    payload_len: csize_t

  OrbeliasCRetrieveResult {.exportc: "orbelias_retrieve_result", bycopy.} = object
    len: csize_t
    hits: ptr OrbeliasCHit
    total_vectors: cint
    scanned: cint
  ...
  OrbeliasCValue {.exportc: "orbelias_value", bycopy.} = object
    data: pointer
    len: csize_t

  OrbeliasCBatchResult {.exportc: "orbelias_batch_result", bycopy.} = object
    len: csize_t
    values: ptr OrbeliasCValue

const
  OrbeliasOk = cint(0)
  OrbeliasErr = cint(-1)
  OrbeliasAbiVersion = cint(2)

var lastError {.threadvar.}: string
var runtimeReady = false
var handles = initTable[pointer, OrbeliasCHandle]()
...
proc ensureHandle(h: pointer): OrbeliasDb =
  if h == nil:
...
proc registerHandle(db: OrbeliasDb): pointer =
  let handle = OrbeliasCHandle(db: db)
  GC_ref(handle)
...
proc toC(id: OrbeliasId): OrbeliasCId =
  let (p, e, s, t) = id.toRaw
  OrbeliasCId(parent: p, epoch: e, seq: s, t_write: t)

proc fromC(id: OrbeliasCId): OrbeliasId =
...
proc orbelias_abi_version(): cint {.exportc, cdecl, dynlib.} =
  OrbeliasAbiVersion

proc orbelias_last_error(): cstring {.exportc, cdecl, dynlib.} =
  lastError.cstring

proc orbelias_init() {.exportc, cdecl, dynlib.} =
  ## Nim runtime initialization. Idempotent for driver setup paths.
  initRuntime()

proc orbelias_open(nodes: cint): pointer {.exportc, cdecl, dynlib.} =
  try:
    initRuntime()
    clearError()
    let db = orbeliasdb.open(int(nodes))
    return registerHandle(db)
  except CatchableError as e:
    setError(e)
    return nil

proc orbelias_open_dir(nodes: cint, dir: cstring): pointer {.exportc, cdecl, dynlib.} =
  ## 永続化つきで開く（設計書 §16）。
  try:
    initRuntime()
    clearError()
    let db = orbeliasdb.open(int(nodes), dataDir = cstringToString(dir, "dir"))
    return registerHandle(db)
  except CatchableError as e:
    setError(e)
    return nil

proc orbelias_connect(peers: cstring): pointer {.exportc, cdecl, dynlib.} =
  ## クラスタへ接続（設計書 §14）。peers = "host:port,host:port,..."
  try:
    initRuntime()
    clearError()
    let db = orbeliasdb.connect(cstringToString(peers, "peers", allowNil = false))
    return registerHandle(db)
  except CatchableError as e:
    setError(e)
    return nil

proc orbelias_connect_auth(peers, username, password, authToken, secretKey,
                        galaxy: cstring): pointer {.exportc, cdecl, dynlib.} =
  ## 認証つきクラスタ接続。NULL は空文字として扱う。
  try:
    initRuntime()
    clearError()
    let db = orbeliasdb.connect(optStr(peers),
...
proc orbelias_connect_auth_tls(peers, username, password, authToken, secretKey,
                            galaxy: cstring, tls: cint, tlsCaFile,
                            tlsServerName: cstring,
                            tlsInsecureSkipVerify: cint): pointer {.exportc, cdecl, dynlib.} =
  ## TLS-aware authenticated cluster connection. TLS requires a OrbeliasDB core
...
proc orbelias_close(h: pointer) {.exportc, cdecl, dynlib.} =
  try:
    initRuntime()
    clearError()
    if h == nil:
...
proc orbelias_now(h: pointer): cdouble {.exportc, cdecl, dynlib.} =
  try:
    clearError()
    ensureHandle(h).now
  except CatchableError as e:
...
proc orbelias_advance(h: pointer, dt: cdouble) {.exportc, cdecl, dynlib.} =
  try:
    clearError()
...
proc orbelias_ring_configure(h: pointer, ring: cstring,
...
proc orbelias_set_galaxy_description(h: pointer, description: cstring): cint
...
proc orbelias_set_ring_description(h: pointer, ring, description: cstring): cint
                                {.exportc, cdecl, dynlib.} =
  try:
    clearError()
...
proc orbelias_put(h: pointer, ring: cstring, data: pointer, len: csize_t,
               outId: ptr OrbeliasCId): cint {.exportc, cdecl, dynlib.} =
  try:
    clearError()
    if outId == nil:
...
proc orbelias_put_codec(h: pointer, ring: cstring, data: pointer, len: csize_t,
                     codec: cint, outId: ptr OrbeliasCId): cint
                     {.exportc, cdecl, dynlib.} =
  try:
    clearError()
    if outId == nil:
...
proc orbelias_put_vec(h: pointer, ring: cstring, data: pointer, len: csize_t,
...
proc orbelias_put_vec_codec(h: pointer, ring: cstring, data: pointer, len: csize_t,
...
proc orbelias_get(h: pointer, id: OrbeliasCId,
               outLen: ptr csize_t): pointer {.exportc, cdecl, dynlib.} =
  ## 見つからなければ nil。返るバッファは orbelias_free で解放すること。
  try:
    clearError()
    if outLen == nil:
...
proc orbelias_get_codec(h: pointer, id: OrbeliasCId, outLen: ptr csize_t,
...
proc orbelias_free(p: pointer) {.exportc, cdecl, dynlib.} =
  if p != nil:
    deallocShared(p)

proc copyPayloadToShared(s: string): OrbeliasCValue =
  result.len = csize_t(s.len)
  result.data = copyStringToShared(s)

proc orbelias_batch_get(h: pointer, ids: ptr OrbeliasCId,
                     idsLen: csize_t): ptr OrbeliasCBatchResult
                     {.exportc, cdecl, dynlib.} =
  try:
    clearError()
    ...
    let db = ensureHandle(h)
    var nimIds = newSeq[OrbeliasId](int(idsLen))
    let rawIds = cast[ptr UncheckedArray[OrbeliasCId]](ids)
    for i in 0 ..< int(idsLen):
      nimIds[i] = fromC(rawIds[i])
    let values = db.batchGet(nimIds)
    result = cast[ptr OrbeliasCBatchResult](allocShared0(sizeof(OrbeliasCBatchResult)))
    result.len = csize_t(values.len)
    if values.len > 0:
      let valueBytes = values.len * sizeof(OrbeliasCValue)
      result.values = cast[ptr OrbeliasCValue](allocShared0(valueBytes))
      let rawValues = cast[ptr UncheckedArray[OrbeliasCValue]](result.values)
...
proc orbelias_batch_get_free(r: ptr OrbeliasCBatchResult) {.exportc, cdecl, dynlib.} =
  if r == nil:
    return
  if r.values != nil:
    let rawValues = cast[ptr UncheckedArray[OrbeliasCValue]](r.values)
...
proc orbelias_query(h: pointer, id: OrbeliasCId, selection: cstring,
                 outLen: ptr csize_t): pointer {.exportc, cdecl, dynlib.} =
  ## 選択取得（GraphQL 風, 設計書 §15）。JSON 文字列の複製バッファを返す
  ## （orbelias_free で解放）。見つからない/エラー時は nil。
  try:
    clearError()
    if outLen == nil:
      raise newException(ValueError, "out_len is nil")
...
proc orbeliasReadPayloadNode(item: OrbeliasRecord): JsonNode =
...
proc orbeliasReadPageJson(page: OrbeliasReadPage): string =
  var items = newJArray()
  for item in page.items:
    let display = orbeliasReadPayloadNode(item)
...
proc orbelias_read_ring_json(h: pointer, ring, filterJson, selection: cstring,
  ...
  try:
    ...
    let s = orbeliasReadPageJson(pageResult)
    outLen[] = csize_t(s.len)
...
proc orbelias_retrieve(h: pointer, vec: ptr cfloat, vecLen: csize_t, ring: cstring,
                    budget, topRings, focus: cint): ptr OrbeliasCRetrieveResult
                    {.exportc, cdecl, dynlib.} =
  try:
    clearError()
    let db = ensureHandle(h)
    ...
    let rr = db.retrieveWithStats(q, ring = optStr(ring),
                                  ...
                                  focus = int(focus))
    result = cast[ptr OrbeliasCRetrieveResult](allocShared0(sizeof(OrbeliasCRetrieveResult)))
    ...
    if rr.hits.len > 0:
      let hitBytes = rr.hits.len * sizeof(OrbeliasCHit)
      result.hits = cast[ptr OrbeliasCHit](allocShared0(hitBytes))
      let rawHits = cast[ptr UncheckedArray[OrbeliasCHit]](result.hits)
...
proc orbelias_retrieve_free(r: ptr OrbeliasCRetrieveResult) {.exportc, cdecl, dynlib.} =
  if r == nil:
    return
  if r.hits != nil:
    let rawHits = cast[ptr UncheckedArray[OrbeliasCHit]](r.hits)
    for i in 0 ..< int(r.len):
...
proc orbelias_atlas(h: pointer, queryVec: ptr cfloat, queryVecLen: csize_t,
...
proc orbelias_locate(h: pointer, id: OrbeliasCId,
...
proc orbelias_next_visit(h: pointer, id: OrbeliasCId,
...
proc orbelias_next_join(h: pointer, a, b: OrbeliasCId): cdouble {.exportc, cdecl, dynlib.} =
  ## 会合しない場合は -1。
  try:
    clearError()
