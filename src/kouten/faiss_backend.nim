## kouten/faiss_backend — optional FAISS vector backend.
##
## This module talks to a small C ABI bridge (`libkouten_faiss.so`). The main
## KoutenDB build does not link FAISS directly; selecting vbFaiss requires the
## bridge library to be available at runtime.

import std/[algorithm, dynlib, os, tables]
import ./[store, vector_backend]

const FaissDynlib = "libkouten_faiss.so"
...
proc candidateLibs(): seq[string] =
  let envPath = getEnv("KOUTEN_FAISS_BRIDGE")
  if envPath.len > 0:
    result.add envPath
...
proc loadFaissApi(): FaissApi =
  ...
  if lib == nil:
    raise newException(ValueError,
      ...
      "or set KOUTEN_FAISS_BRIDGE=/path/to/libkouten_faiss.so")
  result.lib = lib
  result.newIndex = loadSym[FaissNewProc](lib, "kouten_faiss_new")
  result.freeIndex = loadSym[FaissFreeProc](lib, "kouten_faiss_free")
  result.add = loadSym[FaissAddProc](lib, "kouten_faiss_add")
  result.search = loadSym[FaissSearchProc](lib, "kouten_faiss_search")
  cachedApi = result
