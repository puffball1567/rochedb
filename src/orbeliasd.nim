## orbeliasd — OrbeliasDB ノードサーバ（設計書 §14: スケールアウト実装）
##
## usage: orbeliasd --id=0 --peers=127.0.0.1:7301,127.0.0.1:7302,127.0.0.1:7303 [--data=DIR]
...
when not defined(windows):
  import std/posix
import orbelias/[core, store, select, wire, field, auth]
...
const
  ...
  MaxRetrieveBudget =
    when defined(orbeliasTestSmallLimits): 4
    else: 1024
  MaxRetrieveScan =
...
proc printUsage() =
  echo "OrbeliasDB node server"
  echo ""
  echo "Usage:"
  echo "  orbeliasd --id=N --peers=host:port[,host:port...] [options]"
  echo ""
  echo "Options:"
  ...
  echo ""
  echo "Example:"
  echo "  orbeliasd --id=0 --peers=127.0.0.1:7301 --data=/var/lib/orbelias"
...
proc main() =
  ...
  if authUser.len == 0:
    authUser = getEnv("ORBELIAS_USER")
  if authPassword.len == 0:
    authPassword = getEnv("ORBELIAS_PASSWORD")
  if authUser.len == 0 and authPassword.len == 0 and getEnv("ORBELIAS_AUTH_TOKEN").len > 0:
    authUser = "token"
    authPassword = getEnv("ORBELIAS_AUTH_TOKEN")
  if authSecretKey.len == 0:
    authSecretKey = getEnv("ORBELIAS_SECRET_KEY")
  ...
  if tlsCertFile.len > 0 or tlsKeyFile.len > 0:
    ...
    when not defined(ssl):
      raise newException(ValueError, "TLS support requires building orbeliasd with -d:ssl")
  ...
  listener.listen()
  echo "orbeliasd node", id, " listening on ", peers[id].host, ":", peers[id].port,
