## orbelias - OrbeliasDB command-line client
##
## usage:
##   orbelias put [--data=DIR | --peers=host:port,...] --ring=RING [--near=BASE_RING] [--payload=TEXT | --in=FILE] [--codec=raw|json|nif|bif]
##   orbelias get [--data=DIR | --peers=host:port,...] [--ring=RING | --stellar=RING] [--subring=orders,billing] [--filter='{"id":"RAW_ID"}'] [--selection=SEL] [--sort=id|time] [--rsort=id|time]
##   orbelias query [--data=DIR | --peers=host:port,...] --ring=RING [--filter='{"id":"RAW_ID"}' | --id=RAW_ID] --selection=SEL
##   orbelias list-ring [--data=DIR | --peers=host:port,...] --ring=RING [--limit=N] [--cursor=CURSOR]
##   orbelias count-ring [--data=DIR | --peers=host:port,...] --ring=RING
##   orbelias health --peers=host:port,...
##   orbelias metrics --peers=host:port,...
##   orbelias atlas [--data=DIR | --peers=host:port,...]
##   orbelias driver list|info|install [LANG] [--manifest-path=FILE] [--execute]
##   orbelias doctor
...
proc driverRegistry(): seq[DriverInfo] =
  @[
    DriverInfo(
      ...
      repository: "https://github.com/puffball1567/orbeliasdb-rust",
      packageName: "orbeliasdb",
      installHint: "cargo add orbeliasdb",
      notes: "Published on crates.io as orbeliasdb v0.1.3. Wraps the OrbeliasDB C ABI."
    ),
    DriverInfo(
      name: "node",
      status: "published",
      mode: "Node-API C ABI wrapper, TypeScript API",
      repository: "https://github.com/puffball1567/orbeliasdb-js",
      packageName: "orbeliasdb",
      installHint: "npm install orbeliasdb",
      notes: "Published on npm as orbeliasdb v0.1.3. Bun compatibility is tested on the Node-API path, but remains experimental."
    ),
    DriverInfo(
      name: "php",
      status: "published",
      mode: "FFI / C ABI wrapper",
      repository: "https://github.com/puffball1567/orbeliasdb-php",
      packageName: "orbeliasdb/orbeliasdb",
      installHint: "composer require orbeliasdb/orbeliasdb",
      notes: "Published on Packagist as orbeliasdb/orbeliasdb v0.1.2. Wraps the OrbeliasDB C ABI through PHP FFI."
    ),
    DriverInfo(
      name: "cpp",
      status: "repository-released",
      mode: "C++17 C ABI wrapper",
      repository: "https://github.com/puffball1567/orbeliasdb-cpp",
      packageName: "orbeliasdb-cpp",
      installHint: "git clone https://github.com/puffball1567/orbeliasdb-cpp.git",
      notes: "Released as orbeliasdb-cpp v0.1.1. CMake smoke passes in CI; Conan/vcpkg publication is future work."
    ),
    DriverInfo(
      name: "python",
      status: "published",
      mode: "native TCP wire driver",
      repository: "https://github.com/puffball1567/orbeliasdb-python",
      packageName: "orbeliasdb",
      installHint: "python3 -m pip install orbeliasdb",
      notes: "Published on PyPI as orbeliasdb v0.1.3. Pure Python TCP driver."
    ),
    DriverInfo(
      name: "go",
      ...
      packageName: "github.com/orbeliasdb/orbeliasdb-go",
      installHint: "go get github.com/orbeliasdb/orbeliasdb-go",
...
proc cargoManifestPath(manifestPath, projectDir: string): string =
  if manifestPath.len > 0:
    return manifestPath
  let envManifest = getEnv("ORBELIAS_DRIVER_MANIFEST")
  if envManifest.len > 0:
    return envManifest
  var dir = projectDir
  if dir.len == 0:
    dir = getEnv("ORBELIAS_DRIVER_PROJECT")
  if dir.len > 0:
...
proc printCargoAdd(driver: DriverInfo, manifestPath, projectDir: string,
  ...
  if manifest.len == 0:
    ...
    echo "hint: run from a Rust project, pass --manifest-path=FILE, or set ORBELIAS_DRIVER_MANIFEST"
  else:
...
proc runDriver(args: seq[string], manifestPath = "", projectDir = "",
  ...
  if args.len == 0 or args[0] in ["help", "--help", "-h"]:
    echo "Usage:"
    echo "  orbelias driver list"
    echo "  orbelias driver info LANG"
    echo "  orbelias driver install LANG [--manifest-path=FILE] [--project-dir=DIR] [--execute]"
    echo ""
    ...
    echo "--manifest-path, --project-dir, ORBELIAS_DRIVER_MANIFEST, or ORBELIAS_DRIVER_PROJECT."
...
proc defaultDataDir(): string =
  let envData = getEnv("ORBELIAS_DATA")
  if envData.len > 0:
    envData
  else:
    "data"
...
proc cliId(idArg: string): OrbeliasId =
  if idArg.len == 0:
...
proc cliIdString(id: OrbeliasId): string =
  let raw = id.toRaw()
...
proc sortDirectionName(direction: OrbeliasReadSortDirection): string =
  case direction
  of rsAsc: "asc"
  of rsDesc: "desc"

proc paginationName(mode: OrbeliasPaginationMode): string =
  case mode
  of rpOff: "off"
  of rpOn: "on"
...
proc runDoctor() =
  echo "OrbeliasDB setup doctor"
  var ok = true
  ok = checkDir("third_party/faiss", "FAISS source") and ok
  ok = checkFile("third_party/faiss.version", "FAISS pinned version") and ok
  ok = checkFile("lib/liborbelias_faiss.so", "FAISS bridge") and ok

  if fileExists("lib/liborbelias_faiss.so"):
    let lib = loadLib("lib/liborbelias_faiss.so")
    if lib == nil:
      echo "fail FAISS bridge load: lib/liborbelias_faiss.so"
      echo "     Check that FAISS shared libraries are discoverable."
      ok = false
    else:
      unloadLib(lib)
      echo "ok   FAISS bridge load: lib/liborbelias_faiss.so"
  ...
  if ok:
    ...
    echo "  nim c -d:ssl -o:bin/orbelias src/orbelias.nim"
    echo "  bin/orbelias doctor"
    quit(1)
...
proc pickStableRing(db: OrbeliasDb, horizon: float): string =
...
proc findNifAdapterTool(): string =
  let configured = getEnv("ORBELIASDB_NIF_TOOL")
  if configured.len > 0:
  ...
  for candidate in ["orbeliasdb-nif", "nif_file_tool"]:
...
proc tryDecodeBifWithAdapter(data: string): tuple[ok: bool, text: string] =
  ...
  let (inputFile, inputPath) = createTempFile("orbeliasdb-bif-", ".bif", getTempDir())
  let (outputFile, outputPath) = createTempFile("orbeliasdb-nif-", ".nif", getTempDir())
...
proc recordPayloadDisplay(item: OrbeliasRecord, view: string): JsonNode =
...
proc readPageNode(page: OrbeliasReadPage, view: string): JsonNode =
  var items = newJArray()
...
proc timePageNode(page: OrbeliasTimeReadPage, view, sortField: string,
                  sortDirection: OrbeliasReadSortDirection): JsonNode =
  var items = newJArray()
  for item in page.items:
    let display = recordPayloadDisplay(item, view)
    var node = %*{
...
proc stellarPageNode(page: OrbeliasStellarPage, view, sortField: string,
                     sortDirection: OrbeliasReadSortDirection): JsonNode =
  var rings = newJArray()
...
proc readSortOptions(sortArg, rsortArg: string): tuple[field: string, direction: OrbeliasReadSortDirection] =
...
proc readPaginationMode(value: string): OrbeliasPaginationMode =
...
proc runGet(dataDir, peers, username, password, authToken, secretKey,
  ...
  try:
    ...
    let options = OrbeliasReadOptions(
    ...
    if subrings.len > 0 or resolvedId.len == 0:
      let stellar = db.readStellar(ring, OrbeliasStellarOptions(
...
proc runTimeGet(dataDir, peers, username, password, authToken, secretKey,
  ...
  var db = openCliDb(actualDataDir, peers, username, password, authToken, secretKey,
                     ...
                     tlsInsecureSkipVerify = tlsInsecureSkipVerify)
  try:
    let page = db.readTime(ring, fromMs, toMs, OrbeliasReadOptions(
...
proc runShell(dataDir, peers, username, password, authToken, secretKey,
  ...
  var db = openCliDb(actualDataDir, peers, username, password, authToken, secretKey,
                     ...
                     tlsInsecureSkipVerify = tlsInsecureSkipVerify)
  try:
    echo "OrbeliasDB shell. Type help or exit."
    while true:
      stdout.write("orbelias> ")
      stdout.flushFile()
      if stdin.endOfFile():
        break
...
proc runRedisBench(n, payloadBytes: int, redisEndpoint, peers, username,
                   ...
                   tls: bool, tlsCaFile, tlsServerName: string,
                   tlsInsecureSkipVerify: bool) =
  ## Simple KV comparison between localhost Redis GET and OrbeliasDB embedded get.
  ...
  let orbeliasSetUs = float((getMonoTime() - t).inNanoseconds) / 1e3 / float(n)

  t = getMonoTime()
  var orbeliasGot = 0
  for i in 0 ..< n:
    orbeliasGot += db.get(ids[i]).len
  let orbeliasGetUs = float((getMonoTime() - t).inNanoseconds) / 1e3 / float(n)
  doAssert orbeliasGot == n * payload.len
  db.close()

  var orbeliasTcpPutUs = 0.0
  var orbeliasTcpGetUs = 0.0
  var orbeliasTcpBatchGetUs = 0.0
  const PipeBatch = 256
  if peers.len > 0:
    var tcp = connect(peers, username = username, password = password,
                      ...
                      tlsServerName = tlsServerName,
                      tlsInsecureSkipVerify = tlsInsecureSkipVerify)
    var tcpIds = newSeq[OrbeliasId](n)
    t = getMonoTime()
    for i in 0 ..< n:
      tcpIds[i] = tcp.put(payload, ring = "redis-bench")
    orbeliasTcpPutUs = float((getMonoTime() - t).inNanoseconds) / 1e3 / float(n)

    t = getMonoTime()
    var tcpGot = 0
    for i in 0 ..< n:
      tcpGot += tcp.get(tcpIds[i]).len
    orbeliasTcpGetUs = float((getMonoTime() - t).inNanoseconds) / 1e3 / float(n)
    ...
    while start < n:
      ...
      start = stop
    orbeliasTcpBatchGetUs = float((getMonoTime() - t).inNanoseconds) / 1e3 / float(n)
  ...
  let prefix = "orbeliasdb:bench:" & $epochTime() & ":"
  ...
  echo &"  OrbeliasDB embedded put {orbeliasSetUs:8.2f} µs/op ({1e6 / orbeliasSetUs:9.0f} ops/s)"
  echo &"  OrbeliasDB embedded get {orbeliasGetUs:8.2f} µs/op ({1e6 / orbeliasGetUs:9.0f} ops/s)"
  if peers.len > 0:
    echo &"  OrbeliasDB TCP put      {orbeliasTcpPutUs:8.2f} µs/op ({1e6 / orbeliasTcpPutUs:9.0f} ops/s)"
    echo &"  OrbeliasDB TCP get      {orbeliasTcpGetUs:8.2f} µs/op ({1e6 / orbeliasTcpGetUs:9.0f} ops/s)"
    echo &"  OrbeliasDB TCP batch get{orbeliasTcpBatchGetUs:8.2f} µs/op ({1e6 / orbeliasTcpBatchGetUs:9.0f} ops/s)"
  ...
  if peers.len > 0:
    echo &"  get_ratio redis_tcp/orbelias_tcp={redisGetUs / orbeliasTcpGetUs:6.2f}x redis_pipe/orbelias_tcp={redisPipeGetUs / orbeliasTcpGetUs:6.2f}x"
    echo &"  batch_ratio redis_pipe/orbelias_batch={redisPipeGetUs / orbeliasTcpBatchGetUs:6.2f}x"
  echo &"  get_ratio redis_tcp/orbelias_embedded={redisGetUs / orbeliasGetUs:6.2f}x redis_pipe/orbelias_embedded={redisPipeGetUs / orbeliasGetUs:6.2f}x"
...
proc runCompact(dataDir: string, durability: OrbeliasDurability) =
  if dataDir.len == 0:
...
proc runBackup(dataDir, backupDir: string, durability: OrbeliasDurability) =
...
proc runRestoreEncrypted(backupDir, dataDir, passphrase: string,
                         overwrite: bool, durability: OrbeliasDurability) =
  if dataDir.len == 0 or backupDir.len == 0 or passphrase.len == 0:
    raise newException(ValueError,
...
proc recoveryManifest(encrypted: bool, archive, backupFile, universeName,
  ...
  %*{
    "version": 1,
    "kind": "orbeliasdb-recovery-mirror",
    "createdAt": $now(),
    "encrypted": encrypted,
...
proc writeRecoveryManifest(mirror: string, manifest: JsonNode) =
  writeFile(mirror / "orbelias.recovery.json", pretty(manifest))
...
proc runRecoveryBackup(dataDir: string, universes: seq[RecoveryUniverseEntry],
  ...
  try:
    for universeConfig in universes:
      ...
      let backupFile = archive / (if encrypted: "orbelias.backup" else: "orbelias.log")
      let universeName =
...
proc verifyRecoveryMirror(archive, passphrase: string): RecoveryCandidate =
  ...
  let manifestPath = archive / "orbelias.recovery.json"
...
proc replaceRecoveryLog(stageDir, dataDir: string) =
  createDir(dataDir)
  let src = stageDir / "orbelias.log"
  let dst = dataDir / "orbelias.log"
...
proc runRecoveryRestore(archives: seq[string], dataDir, passphrase: string,
  ...
  if fileExists(dst) and not overwrite:
    raise newException(IOError, "target orbelias.log already exists: " & dst)
...
proc parseDurability(value: string): OrbeliasDurability =
  case value
...
proc printHelp() =
  echo "OrbeliasDB command-line client"
  echo ""
  echo "Usage:"
  echo "  orbelias put [--data=DIR | --peers=host:port,...] --ring=RING [--near=BASE_RING] [--payload=TEXT | --in=FILE] [--codec=auto|raw|json|nif|bif]"
  echo "  orbelias get [--data=DIR | --peers=host:port,...] [--ring=RING | --stellar=RING] [--subring=orders,billing] [--filter='{\"id\":\"RAW_ID\"}'] [--selection=SEL] [--limit=N] [--cursor=CURSOR] [--sort=id|time] [--rsort=id|time] [--pagination=on|off] [--page=N] [--pagelimit=N]"
  echo "  orbelias stellar attach|detach|list --stellar=RING [--ring=RING]"
  echo "  orbelias time-orbit --data=DIR --ring=RING [--bucket-ms=N] [--bits=60] [--phase=N] [--salt=TEXT]"
  echo "  orbelias time-put --data=DIR --ring=RING --time-ms=N [--payload=TEXT | --in=FILE]"
  echo "  orbelias time-get --data=DIR --ring=RING --from-ms=N --to-ms=N [--filter=JSON] [--selection=SEL] [--limit=N]"
  echo "  orbelias query [--data=DIR | --peers=host:port,...] --ring=RING [--filter='{\"id\":\"RAW_ID\"}' | --id=RAW_ID] --selection=SEL"
  echo "  orbelias list-ring [--data=DIR | --peers=host:port,...] --ring=RING [--limit=N] [--cursor=CURSOR]"
  echo "  orbelias count-ring [--data=DIR | --peers=host:port,...] --ring=RING"
  echo "  orbelias ring-profile --data=DIR --ring=RING [--codec=raw|json|nif|bif] [--charset=UTF-8] [--format-version=VERSION]"
  echo "  orbelias shell [--data=DIR | --peers=host:port,...]"
  echo "  orbelias atlas [--data=DIR | --peers=host:port,...]"
  echo "  orbelias health|metrics|rings --peers=host:port,..."
  echo "  orbelias driver list|info|install [LANG] [--manifest-path=FILE] [--execute]"
  echo "  orbelias compact --data=DIR"
  echo "  orbelias locality --data=DIR [--metrics]"
  echo "  orbelias backup --data=DIR --backup=DIR [--durability=buffered|strong]"
  echo "  orbelias restore --backup=DIR --data=DIR [--overwrite] [--durability=buffered|strong]"
  echo "  orbelias dump --data=DIR [--out=FILE] [--no-vectors]"
  echo "  orbelias import-jsonl --data=DIR --in=FILE [--ring-field=FIELD] [--default-ring=RING]"
  echo "  orbelias universe-sync --data=SOURCE_DIR [--target-data=TARGET_DIR | --peers=host:port,...] [--prune-acked]"
  echo "  orbelias universe-status [--data=DIR | --peers=host:port,...] [--metrics]"
  echo "  orbelias recovery-status [--mirror=DIR...] [--universe-config=FILE] [--required-healthy=N] [--metrics]"
  echo "  orbelias doctor"
  echo ""
  echo "ID formats:"
  echo "  parent:seq"
  echo "  parent:epoch:seq:tWrite"
  echo ""
  echo "Local commands use --data=DIR, ORBELIAS_DATA, or ./data by default."
  echo "Cluster commands use --peers=host:port,... or --config=FILE."
  echo "ORBELIAS_CONFIG can point to the same JSON config file."
  echo "Auth can use --user/--password-file/--secret-key-file or ORBELIAS_USER, ORBELIAS_PASSWORD, ORBELIAS_SECRET_KEY, ORBELIAS_AUTH_TOKEN."
...
proc main() =
  ...
  var configPath = getEnv("ORBELIAS_CONFIG")
  var universeConfig = ""
  ...
  if username.len == 0:
    username = getEnv("ORBELIAS_USER")
  if password.len == 0:
    password = getEnv("ORBELIAS_PASSWORD")
  if authToken.len == 0:
    authToken = getEnv("ORBELIAS_AUTH_TOKEN")
  if secretKey.len == 0:
    secretKey = getEnv("ORBELIAS_SECRET_KEY")
